...
mod analysis_review;
mod engine_assets;
mod engine_model_catalog;
mod zhizi;
mod live_transport;
mod live_analysis;
use live_analysis::{LiveReviewSession, tear_down_live_review, tear_down_live_review_with_app};
#[cfg(test)]
use live_analysis::*;
mod try_play;
...
/// Which engine lifecycle owns a registry slot.
///
/// The kind is what makes cross-kind exclusion possible: a batch start must know whether the slot
/// it would take is held by a live session, and a finishing worker must know whether the slot it is
/// about to drop is still its own (a newer job of the other kind may already own the key).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum AnalysisJobKind {
    /// The live-review GTP session (`katago_start_live_review`).
    Live,
    /// A spawned batch analysis worker (`katago_start_analyze_game`).
    Batch,
}

impl AnalysisJobKind {
    fn label(self) -> &'static str {
        match self {
            Self::Live => "live review",
            Self::Batch => "batch analysis",
        }
    }
}
...
/// Registry of running analysis jobs: `job_id` -> (kind, cancel token).
///
/// The registry does **not** schedule or recover work — the frontend owns the intent to restart.
/// It answers three questions only: who owns a slot, how to cancel it, and whether a slot is still
/// the entry the caller put there.
#[derive(Default)]
struct AnalysisJobRegistry {
    jobs: Mutex<HashMap<String, AnalysisJobEntry>>,
    /// Serializes the *sequencing* of one engine lifecycle after another.
    ///
    /// Taking the exclusive slot is not enough on its own: replacing a live session with a batch
    /// job (or with another live session) means "stop the old one, wait for it to really exit, then
    /// register the new one". Two starts interleaving in that window could otherwise both find the
    /// slot free after one tear-down and both claim it. Every start/stop path holds this lock while
    /// it makes that decision, and **never** holds the `jobs` mutex across a join, so a returning
    /// worker can always drop its own entry.
    lifecycle: Mutex<()>,
}

/// The token handed to a job together with its exclusive slot.
///
/// The token *is* the slot's identity: it is minted per job, the registry keeps one clone (for
/// `cancel`) and the worker holds another, and the two are pointer-identical. `remove_if_owner`
/// therefore needs no second id — a mismatched token simply is not the stored entry.
#[derive(Clone, Debug)]
struct AnalysisJobSlot {
    cancel_token: AnalysisCancelToken,
}

impl AnalysisJobRegistry {
    /// Take the single exclusive slot, failing if any job already holds it.
    ///
    /// Callers that may need to displace an existing job (the two `start` commands) tear the old
    /// job down under [`Self::lifecycle`] *before* calling this, so "the slot is empty" is the only
    /// precondition this needs to check.
    fn try_insert_exclusive(&self, kind: AnalysisJobKind, job_id: String) -> Result<AnalysisJobSlot, String> {
        let mut jobs = self
            .jobs
            .lock()
            .map_err(|_| "analysis job registry is unavailable".to_string())?;
        if let Some(existing) = jobs.values().next() {
            return Err(format!(
        ...
        Ok(AnalysisJobSlot { cancel_token })
    }

    /// Cancel the job in `job_id`, leaving its slot in place.
    ///
    /// The slot is *not* removed here: a cancelled batch worker may still be tearing its engine
    /// down, and dropping the slot early would both reopen the exclusive slot to a new job and make
    /// the worker's own [`Self::remove_if_owner`] a no-op on the wrong entry. The token stays,
    /// already cancelled, so a repeated cancel is observable and idempotent; the worker releases
    /// the slot when it actually returns.
    ...
    /// Release a slot only if it is still the one the caller took.
    ///
    /// This is the guard against "a finishing worker removes a newer job": a live session torn down
    /// after a replacement started, or a batch worker finishing late, must not evict a slot that a
    /// different job now owns. The token *is* the identity, so no separate id is needed.
    fn remove_if_owner(&self, job_id: &str, slot: &AnalysisJobSlot) -> bool {
        let Ok(mut jobs) = self.jobs.lock() else {
            return false;
        };
        match jobs.get(job_id) {
            Some(entry) if entry.cancel_token.is_same_as(&slot.cancel_token) => {
                jobs.remove(job_id);
                true
            }
            _ => false,
    ...
    /// Release a slot unconditionally.
    ///
    /// Only for the live tear-down path, which owns the key by construction: the job id comes out of
    /// the session being torn down, and the session state was already emptied under the lifecycle
    /// lock, so no other live start can be holding that key. Batch workers must use
    /// [`Self::remove_if_owner`] instead.
    ...
    /// The kind of whichever job currently owns the exclusive slot, if any.
    ///
    /// This is the sequence gate for the start paths: it says what a start would have to displace,
    /// and it is only meaningful while the caller holds [`Self::lifecycle`].
    fn kind_of_first(&self) -> Option<AnalysisJobKind> {
        self.jobs
            .lock()
            .ok()
            .and_then(|jobs| jobs.values().next().map(|entry| entry.kind))
    }

    /// Cancel every job still registered, keeping the slots. Used by the process-exit path so a
    /// second window closing cannot orphan a job another window still owns.
    fn cancel_all(&self) -> Result<Vec<String>, String> {
        let jobs = self
            .jobs
            .lock()
            .map_err(|_| "analysis job registry is unavailable".to_string())?;
...
fn engine_error_kind(err: &EngineManagerError) -> &'static str {
    match err {
        ...
        // 长驻 GTP 会话（plan 004）新增的错误变体。该 match 无通配臂，新增变体会让
        // 本 crate 编译失败（E0004）；而 engine-manager 无法在不越界改 apps/ 的前提下
        // 修复，会把 plan 004 的 --workspace 门禁死锁。此处由 captain 统一补齐映射。
        EngineManagerError::SessionNotRunning => "sessionNotRunning",
        EngineManagerError::SessionExited { .. } => "sessionExited",
        EngineManagerError::SessionWrite { .. } => "sessionWrite",
        EngineManagerError::SessionStopUnconfirmed => "sessionStopUnconfirmed",
    }
}
...
#[tauri::command]
fn validate_sgf_tree_legality(sgf_text: String) -> Result<(), String> { sgf::edit_validation::validate_sgf_tree_legality(&sgf_text).map_err(|e| e.to_string()) }
...
/// Grade the analysed frames and return the problem moves the UI lists.
///
/// `sgf_text` is the SGF the frames were produced from. It is **required** for a real review: only
/// the SGF carries the real node identity, parent/child links and its own turn convention, so the
/// frames alone cannot say which node a turn belongs to or who actually played it. Passing `None`
/// keeps the legacy call shape working (old persisted payloads and the browser fallback still call
/// this with frames only) and returns no problems rather than a guess - a missing document must
/// never be reported as "no problems found".
#[tauri::command]
fn classify_problems(
    frames: Vec<AnalysisFrameDto>,
    sgf_text: Option<String>,
) -> Result<Vec<app_model::ProblemMarkerDto>, String> {
    match sgf_text {
        Some(sgf_text) => analysis_review::classify_sgf_problems(&sgf_text, &frames),
        None => Ok(Vec::new()),
    }
}
...
fn save_engine_profiles_settings(
    ...
    let _writes = benchmark_settings::PROFILE_WRITES.lock().map_err(|_| "配置状态不可用")?;
    let path = engine_profile_path(&app_handle)?;
...
async fn katago_analyze_once(
    ...
    // This command spawns an engine, so it **occupies** the exclusive slot for as long as its engine
    // work runs, instead of merely checking that the slot is empty. A check-only version is not
    // mutual exclusion: two concurrent `analyze_once` calls would each observe an empty slot before
    // the other registered anything, and both would spawn an engine. The 60s timeout bounds how
    // long the slot is held; it is not a substitute for holding it.
    let slot = claim_engine_slot(&app_handle, AnalysisJobKind::Batch)?;
    analyze_once_frame(
        profile,
        sgf_text,
        turn,
        max_visits,
        EngineSlotGuard::new(&app_handle, slot),
    )
    .await
}

/// The engine work of `katago_analyze_once`.
///
/// Kept separate so the non-blocking regression test can drive the real future by hand without
/// needing a Tauri app handle: the slot rule is proven on the registry itself, and this function is
/// the part whose inline-vs-deferred behaviour the test measures. `slot_guard` is the exclusive slot
/// the caller claimed; it is moved into the blocking closure so the slot is held for exactly the
/// engine work's lifetime and released before this future resolves.
async fn analyze_once_frame(
    profile: EngineProfileDto,
    sgf_text: String,
    turn: u32,
    max_visits: u32,
    slot_guard: EngineSlotGuard,
) -> Result<AnalysisFrameDto, String> {
    // Analysis spawns an engine and can block for up to 60s. As a synchronous command that work
    // runs inline on the IPC handler and freezes the whole UI; as an `async fn` Tauri dispatches
    // the future to its runtime instead. The blocking engine loop is moved onto a blocking thread
    // so it never occupies a runtime worker. The command signature and return type are unchanged,
    // so existing `invoke` callers keep working.
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let query = katago_protocol::analysis_query_from_game(
            ...
            AnalysisQueryOptions {
                id: job_id.to_string(),
                rules: sgf::katago_rules_for_sgf_ru(game.summary.rules.as_deref()).to_string(),
                turn,
...
/// Claim the exclusive engine slot for a single-shot command, or report who holds it.
///
/// Unlike the registered jobs (`katago_start_*`), a single-shot command has no cancel token the
/// frontend knows about, so it takes a throwaway slot keyed by a fresh id: the only thing it needs
/// from the registry is mutual exclusion. The key is returned alongside the slot so the caller can
/// release it through [`EngineSlotGuard`].
///
/// A missing registry is not an error: the command is then running outside a Tauri app, where no
/// job can hold a slot. In that case there is nothing to exclude against and the guard is inert.
fn claim_engine_slot(
    app_handle: &AppHandle,
    kind: AnalysisJobKind,
) -> Result<(String, Option<AnalysisJobSlot>), String> {
    let Some(registry) = app_handle.try_state::<AnalysisJobRegistry>() else {
        return Ok((String::new(), None));
    };
    claim_single_shot_slot(&registry, kind).map(|(job_id, slot)| (job_id, Some(slot)))
}

/// The app-handle-free half of [`claim_engine_slot`], so the rule is testable without a Tauri app.
///
/// A single-shot run never displaces a registered live session or batch job: the frontend would lose
/// work the user is watching. It only proceeds when the slot is genuinely empty, and it then *takes*
/// the slot so a concurrent single-shot run cannot slip in behind it.
///
/// The claim is made under [`AnalysisJobRegistry::lifecycle`], the same lock every start/stop path
/// holds, and it is a *single* atomic step: "is the slot empty" is decided by
/// [`AnalysisJobRegistry::try_insert_exclusive`], not by a separate look at
/// [`AnalysisJobRegistry::kind_of_first`]. A check-then-claim pair is not mutual exclusion against a
/// tear-down: `start_batch_analysis` releases the displaced live session's slot when the owner
/// returns, which is *before* the join completes and long before the batch slot is claimed. A
/// single-shot run that only checked first would observe that empty window, take the `Batch` slot,
/// and then make the batch start's own claim fail — and its debug assertion panic while still
/// holding `lifecycle`, poisoning the lock for every later start. Holding `lifecycle` for the claim
/// itself means a single-shot run either wins the slot before the tear-down begins (and the batch
/// start then sees a non-live owner and refuses cleanly) or loses it to the batch start's claim.
fn claim_single_shot_slot(
    registry: &AnalysisJobRegistry,
    kind: AnalysisJobKind,
) -> Result<(String, AnalysisJobSlot), String> {
    let _lifecycle = registry
        .lifecycle
        .lock()
        .map_err(|_| "analysis job lifecycle is unavailable".to_string())?;
    let job_id = format!("single-shot-{}", Uuid::new_v4());
    let slot = registry.try_insert_exclusive(kind, job_id.clone())?;
    Ok((job_id, slot))
}

/// Holds a single-shot command's exclusive slot for as long as the value lives.
///
/// The guard is what makes the exclusion real: the command claims the slot *before* awaiting, and the
/// slot is released when the guard is dropped — inside the blocking closure that ran the engine, so
/// it is still held for every exit path (success, engine error, parse error, panic unwinding).
struct EngineSlotGuard {
    app_handle: Option<AppHandle>,
    job_id: String,
    slot: Option<AnalysisJobSlot>,
}

impl EngineSlotGuard {
    /// Build the guard from [`claim_engine_slot`]'s result.
    fn new(app_handle: &AppHandle, (job_id, slot): (String, Option<AnalysisJobSlot>)) -> Self {
        Self {
            app_handle: slot.as_ref().map(|_| app_handle.clone()),
            job_id,
            slot,
        }
    }

    /// An inert guard: no slot, nothing to release.
    ///
    /// Only for the tests that drive [`analyze_once_frame`] / [`analyze_game_frames`] by hand without
    /// a Tauri app; the slot rule itself is proven against the registry, not through these tests.
    #[cfg(test)]
    fn inert() -> Self {
        Self {
            app_handle: None,
...
impl Drop for EngineSlotGuard {
    fn drop(&mut self) {
        let (Some(app_handle), Some(slot)) = (self.app_handle.as_ref(), self.slot.as_ref()) else {
            return;
        };
        if let Some(registry) = app_handle.try_state::<AnalysisJobRegistry>() {
            let _ = registry.remove_if_owner(&self.job_id, slot);
        }
    }
}

#[tauri::command]
async fn katago_analyze_game(
    app_handle: AppHandle,
    profile: EngineProfileDto,
    sgf_text: String,
    max_visits: u32,
) -> Result<Vec<AnalysisFrameDto>, String> {
    // Same rule as `katago_analyze_once`: this is a table-driven batch variant, and it **occupies**
    // the slot for its whole run rather than only checking that it is empty at entry.
    let slot = claim_engine_slot(&app_handle, AnalysisJobKind::Batch)?;
    analyze_game_frames(
        profile,
        sgf_text,
...
/// The engine work of `katago_analyze_game` (see [`analyze_once_frame`] for the split, including
/// why `slot_guard` is moved in rather than read from the outside).
async fn analyze_game_frames(
    profile: EngineProfileDto,
    sgf_text: String,
    max_visits: u32,
    slot_guard: EngineSlotGuard,
) -> Result<Vec<AnalysisFrameDto>, String> {
    // Same reasoning as `katago_analyze_once`: this batch path can block for up to 600s, so it must
    // not run inline on the IPC handler.
    tauri::async_runtime::spawn_blocking(move || {
        let _slot_guard = slot_guard;
        let job_id = Uuid::new_v4();
...
fn katago_start_analyze_game(
    app_handle: AppHandle,
    live_state: State<'_, Mutex<LiveReviewSession>>,
    ...
    // Starting a batch means "analyse this game to the end", which cannot coexist with a live
    // session on the same single engine budget. `start_batch_analysis` displaces a running live
    // session and only then claims the slot, all under the lifecycle lock (see its docs).
    let slot = start_batch_analysis(&live_state, &registry, job_id_string.clone())?;

    std::thread::spawn({
        let job_id_string = job_id_string.clone();
        move || {
            run_katago_analysis_job(app_handle, job_id, job_id_string, spec, prepared, slot);
        }
    });
...
/// Take the exclusive batch slot, displacing a live session if one is running.
///
/// Batch and live cannot share the one engine budget, so a batch start is allowed to *replace* a
/// live session (the UI's "analyse this game to the end" after a review). The order below is the
/// whole point, and it is serialized by [`AnalysisJobRegistry::lifecycle`]:
///
/// 1. **under the lifecycle lock — decide.** Look at what owns the slot. A live session is going to
///    be displaced; a batch job is refused, because the caller cannot distinguish its own running
///    batch from someone else's, and replacing it would silently kill work the user is watching.
/// 2. **still under the lifecycle lock — stop the old live session and confirm it exited.** Detach
///    it from the state, signal it, and join its owner thread. The join is what makes "the slot is
///    free" true rather than likely: the owner's own `remove_if_owner` runs before it returns, so by
///    the time the join completes the entry is gone. The `jobs` mutex is never held across the join
///    (the owner needs it to release its entry), which is why the two mutexes are distinct.
/// 3. **claim the new slot.** The slot is genuinely empty now, so no reservation scheme and no
///    second identity are needed.
///
/// Holding the lifecycle lock across all three steps is what stops two concurrent starts from both
/// finding the slot free after one tear-down and both claiming it.
///
/// The claim is made *once*, here, and is not preceded by a re-read of the registry: step 1 already
/// established that anything in the slot but a `Live` entry was refused, and the only entry step 2
/// can have removed is that live session. A second look (a `kind_of_first` assertion, say) would not
/// be a claim anyway — it observes a slot it does not hold, and its failure mode is a panic while
/// the `lifecycle` lock is held, which poisons the lock and strands every later start. The
/// single-shot path takes the same `lifecycle` lock, so nothing can enter the slot between step 2's
/// teardown and this claim; if it did, the claim simply fails and the caller gets an error.
fn start_batch_analysis(
    live_state: &Mutex<LiveReviewSession>,
    registry: &AnalysisJobRegistry,
    job_id: String,
) -> Result<AnalysisJobSlot, String> {
    let _lifecycle = registry
        .lifecycle
        .lock()
        .map_err(|_| "analysis job lifecycle is unavailable".to_string())?;
    ...
    // Undo any live session: detach under the state lock, then signal/join/release with it free.
    tear_down_live_review(live_state, registry);

    registry.try_insert_exclusive(AnalysisJobKind::Batch, job_id)
}
...
fn cancel_analysis_job_in_registry(registry: &AnalysisJobRegistry, job_id: &str) -> Result<bool, String> {
    ...
    // Cancellation signals the token only. The slot stays until the worker really returns, so the
    // exclusive slot cannot be reopened while the previous engine is still exiting.
...
fn run_katago_analysis_job(
    ...
    slot: AnalysisJobSlot,
) {
    let cancel_token = slot.cancel_token.clone();
    let mut on_progress = {
    ...
    // The worker is the only place that stops when the job is *really* over: the engine process has
    // exited and the batch runner has returned, cancelled or not. A cancel request only marks the
    // token, so the exclusive slot stays taken until this point rather than being reopened while the
    // old engine is still winding down.
    ...
    // The engine process is gone by now (`run_katago_analysis_batch_with_options` returns only after
    // the child has been reaped, cancelled or not), so the slot is free for the next job from this
    // point. Release it *before* the terminal event, on every path: a frontend that reacts to
    // `analysis-complete` / `analysis-error` / `analysis-cancelled` by immediately starting the next
    // job would otherwise be refused by a slot still held by a job whose engine has already exited.
    release_batch_slot(&app_handle, &job_id_string, &slot);
...
/// Release a batch job's exclusive slot if it is still the one this worker took.
///
/// A superseded worker must not evict its successor's entry, so this is always token-checked.
fn release_batch_slot(app_handle: &AppHandle, job_id_string: &str, slot: &AnalysisJobSlot) {
    if job_id_string.starts_with("curve-") {
        if let Some(registry) = app_handle.try_state::<background_curve::CurveRegistry>() {
            registry.0.remove_if_owner(job_id_string, slot);
        }
        return;
    }
    if let Some(registry) = app_handle.try_state::<AnalysisJobRegistry>() {
        let _ = registry.remove_if_owner(job_id_string, slot);
    }
}
...
pub fn run() {
    tauri::Builder::default()
        .manage(engine_assets::AssetDownloads::default())
        .manage(benchmark_settings::BenchmarkResults::default())
        .manage(zhizi::ZhiziState::default())
        ...
        .on_window_event(|window, event| {
            // Closing the last window must not leave the engine process behind: the owner thread
            // holds the `GtpSession`, and `LiveReviewSession` lives on the app, so relying on a
            // `Drop` would not run before the process is reaped.
            if let tauri::WindowEvent::Destroyed = event {
                tear_down_live_review_with_app(&window.app_handle().clone());
            }
        })
        .invoke_handler(tauri::generate_handler![
            online_kifu::fetch_online_kifu,
            benchmark_settings::setup_benchmark_apply,
            setup_model_probe::setup_model_probe,
            setup_model_probe::setup_model_probe_cancel,
            setup_apply::setup_apply_profiles,
            engine_benchmark::setup_benchmark_start,
            engine_benchmark::setup_benchmark_cancel,
            health,
            ...
            background_curve::start_background_curve,
            background_curve::cancel_background_curve,
            katago_cancel_analysis,
            engine_assets::engine_hardware,
            engine_assets::engine_release_catalog,
            engine_model_catalog::engine_model_catalog,
            engine_assets::engine_assets_list,
            engine_assets::engine_asset_install,
            engine_assets::engine_asset_import,
            engine_assets::engine_asset_cancel,
            engine_assets::engine_asset_remove,
            engine_assets::engine_asset_cleanup,
            live_analysis::ssh_start_live_review,
            ...
            validate_sgf_tree_legality,
            zhizi::zhizi_status,
            live_analysis::zhizi_logout,
            live_analysis::zhizi_start_live_review,
            live_analysis::katago_start_live_review,
            live_analysis::katago_live_review_set_position,
            live_analysis::katago_live_review_stop,
            live_analysis::katago_live_review_pause,
            live_analysis::katago_live_review_configure
        ])
        .build(tauri::generate_context!())
        .expect("error while building the Tauri application")
        .run(|app, event| {
            // Belt and braces for exit paths that do not destroy a window first (Cmd-Q, `Exit`).
            // The live session is joined here; batch workers are signalled only, because joining a
            // 600s engine run would hang the exit, and the process is going away regardless.
            if let tauri::RunEvent::ExitRequested { .. } | tauri::RunEvent::Exit = event {
                tear_down_live_review_with_app(app);
...
mod tests {
    ...
    /// Write a `/bin/sh` fake engine script and return a GTP profile pointing at it.
    ///
    /// Used to exercise the analysis commands without a real KataGo binary. `engine_path` must be
    /// absolute so `build_command_spec` resolves it verbatim.
    #[cfg(unix)]
    fn fake_gtp_profile(dir: &Path, script: &str) -> EngineProfileDto {
        use std::os::unix::fs::PermissionsExt;

        fs::create_dir_all(dir).unwrap();
        let engine_path = dir.join("fake-katago.sh");
        fs::write(&engine_path, format!("#!/bin/sh\n{script}\n")).unwrap();
        fs::set_permissions(&engine_path, fs::Permissions::from_mode(0o755)).unwrap();

        let model_path = dir.join("model.bin");
        let config_path = dir.join("gtp.cfg");
    ...
    #[cfg(unix)]
    fn fake_gtp_profile_for_sgf(dir: &Path, script: &str, sgf_text: &str) -> (EngineProfileDto, String) {
        (fake_gtp_profile(dir, script), sgf_text.to_string())
    }

    /// Drive a live [`engine_manager::GtpSession`] against a fake engine that echoes every command
    /// back on stdout, and return the commands the app actually wrote.
    ///
    /// Asserting on the *echoed* lines (rather than a file the shell writes) makes the test
    /// deterministic: a file-based log races the fake engine's exit and flakes.
    #[cfg(unix)]
    #[test]
    fn live_review_command_ids_cover_the_whole_batch_and_only_the_analyze_opens_the_stream() {
        let ids = LiveReviewCommandIds { first: 5, analyze: 9 };
        // Preparation commands share the range, so an error on any of them is attributable.
        for id in 5..=9 {
            assert!(ids.contains(id), "id {id} belongs to this request");
        }
        assert!(!ids.contains(4), "an earlier request's id must not match");
        assert!(!ids.contains(10), "a later request's id must not match");

        // Only the analyze acknowledgement is the boundary; a `?` on a preparation command must
        // not open the stream, which is the bug the range was added to close.
        assert!(ids.is_boundary(9));
        assert!(!ids.is_boundary(5), "boardsize's ack is not the boundary");
        assert!(!ids.is_boundary(7), "a play ack is not the boundary");
    }

    #[test]
    fn setup_pairs_render_for_set_position_and_reject_malformed_input() {
        let rendered =
            setup_pairs_to_set_position(&["B".into(), "D4".into(), "W".into(), "G3".into()]).unwrap();
        assert_eq!(rendered, "B D4 W G3");

        // An odd-length list would silently place nothing, so it must fail here.
        assert!(setup_pairs_to_set_position(&["B".into(), "D4".into(), "W".into()]).is_err());
        assert!(setup_pairs_to_set_position(&["B".into(), "  ".into()]).is_err());
        assert_eq!(setup_pairs_to_set_position(&[]).unwrap(), "");
    }

    #[cfg(unix)]
    #[test]
    fn live_review_setup_position_uses_set_position_and_rules() {
        // A layout position: stones via `set_position` (no invented move history), rules via
        // `kata-set-rules`, and `PL[W]` honoured by consuming a virtual Black pass.
        let dir = native_config_temp_dir("live-review-setup");
        let profile = fake_gtp_profile(&dir, "while read -r line; do printf '%s\\n' \"$line\"; done");
        ...
        let request = LiveReviewPositionRequest {
            board_size: 9,
            setup: vec!["B".into(), "D4".into(), "W".into(), "G3".into()],
            moves: Vec::new(),
            rules: Some("japanese".into()),
            handicap: None,
            komi: Some(7.5),
            player: Some("W".into()),
        };
        let mut command_id = 1u64;
        apply_live_review_position(&mut session, &request, 10, &mut command_id).unwrap();

        let mut bodies: Vec<String> = Vec::new();
        let deadline = std::time::Instant::now() + Duration::from_secs(5);
        while std::time::Instant::now() < deadline {
            match session.next_event_timeout(Duration::from_millis(100)) {
                Some(engine_manager::GtpSessionEvent::Line(line)) => {
                    if let Some((_, body)) = split_live_command(&line) {
                        let is_analyze = body.starts_with("kata-analyze");
                        bodies.push(body);
                        if is_analyze {
                            break;
                        }
                ...
                | Some(engine_manager::GtpSessionEvent::ReadError(_)) => break,
                None => {}
            }
        }
        let _ = session.close();
        ...
        assert!(
            bodies.iter().any(|b| b == "set_position B D4 W G3"),
            "setup stones must reach the engine as set_position: {bodies:?}"
        );
        assert!(
            !bodies.iter().any(|b| b.starts_with("play B D4")),
            "setup stones must not be replayed as moves: {bodies:?}"
        );
        assert!(
            bodies.iter().any(|b| b == "kata-set-rules japanese"),
            "rules must be forwarded or the evaluation uses the wrong ruleset: {bodies:?}"
        );
        assert!(
            !bodies.iter().any(|b| b == "play B pass"),
            "no virtual pass may be invented: `player` selects the search side and a fake pass \
             would pollute the history: {bodies:?}"
        );
        assert!(
            bodies
                .iter()
                .any(|b| b == "kata-analyze W 10 ownership true rootInfo true"),
            "the side to move must be expressed through the analyze player argument: {bodies:?}"
        );
        assert!(
            !bodies.iter().any(|b| b == "clear_board"),
            "a setup position must not be preceded by clear_board: {bodies:?}"
        );
        assert_eq!(
            bodies.last().map(String::as_str),
            Some("kata-analyze W 10 ownership true rootInfo true"),
        );
    }

    #[test]
    fn handicap_validation_rejects_anything_that_would_change_the_position_silently() {
        let base = |setup: Vec<String>, handicap: Option<u32>| LiveReviewPositionRequest {
            board_size: 9,
            setup,
            moves: Vec::new(),
            rules: None,
        ...
        // Happy path: HA=3 with three Black stones.
        let ok = live_review_handicap_stones(&base(
            vec![
                "B".into(),
                "D4".into(),
                "B".into(),
        ...
        .expect("a well-formed handicap must be accepted");
        assert_eq!(ok, vec!["D4", "G3", "C3"]);

        // HA < 2 is not a handicap game, so no handicap stones are emitted at all.
        assert!(
            live_review_handicap_stones(&base(vec!["B".into(), "D4".into()], Some(1)))
                .unwrap()
                .is_empty()
        );
        assert!(
            live_review_handicap_stones(&base(vec!["B".into(), "D4".into()], None))
                .unwrap()
                .is_empty()
        );

        // A White stone in a handicap setup: `set_free_handicap` only takes Black stones, so
        // forwarding this would silently drop it and analyse a different board.
        assert!(live_review_handicap_stones(&base(
            vec!["B".into(), "D4".into(), "W".into(), "G3".into()],
            Some(2)
        ))
        .is_err());

        // Odd number of tokens = an incomplete pair.
        assert!(
            live_review_handicap_stones(&base(vec!["B".into(), "D4".into(), "B".into()], Some(2))).is_err()
        );

        // Declared count disagrees with the stones actually supplied.
        assert!(live_review_handicap_stones(&base(
            vec!["B".into(), "D4".into(), "B".into(), "G3".into()],
            Some(3)
        ))
        .is_err());
        ...
        // Empty vertex.
        assert!(live_review_handicap_stones(&base(
            vec!["B".into(), "  ".into(), "B".into(), "G3".into()],
            Some(2)
        ))
        .is_err());
    }
    ...
    #[cfg(unix)]
    #[test]
    fn live_review_handicap_game_uses_set_free_handicap_not_set_position() {
        // HA >= 2 must be installed with the standard GTP handicap command: only that tells the
        // engine the stones are a handicap, which `whiteHandicapBonus` (Chinese rules) depends on.
        let dir = native_config_temp_dir("live-review-handicap");
        let profile = fake_gtp_profile(&dir, "while read -r line; do printf '%s\\n' \"$line\"; done");
        let spec = build_command_spec(&profile).unwrap();
        let mut session = engine_manager::GtpSession::start(&spec).unwrap();
        ...
        assert!(
            bodies.iter().any(|b| b == "set_free_handicap D4 G3 C3"),
            "a handicap game must use set_free_handicap: {bodies:?}"
        );
        assert!(
            !bodies.iter().any(|b| b.starts_with("set_position")),
            "set_position must not be used for handicap stones: {bodies:?}"
        );
        let clear = bodies.iter().position(|b| b == "clear_board").unwrap();
        let handicap = bodies
            .iter()
            .position(|b| b.starts_with("set_free_handicap"))
            .unwrap();
        assert!(
            clear < handicap,
            "clear before placing handicap stones: {bodies:?}"
        );
        assert!(!bodies[handicap + 1..].iter().any(|b| b == "clear_board"));
    }

    #[cfg(unix)]
    #[test]
    fn live_review_normal_position_replays_moves_without_set_position() {
        // The plain-game path must stay byte-for-byte the same shape as before this change.
        let dir = native_config_temp_dir("live-review-plain");
        let profile = fake_gtp_profile(&dir, "while read -r line; do printf '%s\\n' \"$line\"; done");
        let spec = build_command_spec(&profile).unwrap();
        let mut session = engine_manager::GtpSession::start(&spec).unwrap();
        ...
        assert!(bodies.iter().any(|b| b == "clear_board"), "{bodies:?}");
        assert!(bodies.iter().any(|b| b == "play B D4"), "{bodies:?}");
        assert!(bodies.iter().any(|b| b == "play W Q16"), "{bodies:?}");
        assert!(bodies.iter().any(|b| b == "kata-set-rules chinese"), "{bodies:?}");
        assert!(
            !bodies.iter().any(|b| b.starts_with("set_position")),
            "a plain game must not use set_position: {bodies:?}"
        );
        assert!(
            !bodies.iter().any(|b| b == "play B pass"),
            "a normal position must not consume a virtual pass: {bodies:?}"
        );
        assert_eq!(
            bodies.last().map(String::as_str),
            Some("kata-analyze B 10 ownership true rootInfo true"),
        );
    }

    /// Split a live-review command line into its GTP command id and the command body.
    ///
    /// Returns `None` when the line does not start with an id, which is exactly the regression this
    /// guards: the id must be added in one place only.
    fn split_live_command(line: &str) -> Option<(u64, String)> {
        let (head, rest) = line.split_once(' ')?;
        let id: u64 = head.parse().ok()?;
        Some((id, rest.to_string()))
    }

    fn echoed_live_review_lines(
        name: &str,
        board_size: u8,
        komi: Option<f32>,
        moves: Vec<String>,
        ...
        let dir = native_config_temp_dir(name);
        let profile = fake_gtp_profile(&dir, "while read -r line; do printf '%s\\n' \"$line\"; done");
        let spec = build_command_spec(&profile).unwrap();
        let mut session = engine_manager::GtpSession::start(&spec).unwrap();
        ...
        apply_live_review_position(&mut session, &request, interval_centisec, &mut command_id).unwrap();
        ...
        while std::time::Instant::now() < deadline {
            match session.next_event_timeout(Duration::from_millis(100)) {
                Some(engine_manager::GtpSessionEvent::Line(line)) => {
                    ...
                    let is_analyze = line.starts_with("kata-analyze");
                    lines.push(line);
                    if is_analyze {
                        break;
                    }
    ...
    /// The strongest check available without a real engine: drive a live [`engine_manager::GtpSession`]
    /// against a fake engine that echoes its input, and assert the *actual commands* sent.
    ///
    /// The pure-function test above proves the command string is correct; this test proves the
    /// command really reaches the engine, in the right order, and that both silent-failure flags
    /// (`ownership true`, `rootInfo true`) survive the trip. It does not exercise real
    /// `kata-analyze` output or event delivery — those need the real engine.
    #[cfg(unix)]
    #[test]
    fn live_review_position_sends_board_moves_and_flagged_kata_analyze_to_the_engine() {
        let lines = echoed_live_review_lines(
            "live-review-wire",
            9,
            Some(7.5),
            vec!["B".into(), "E5".into(), "W".into(), "E3".into()],
            Some("B"),
            20,
        );

        // Every command carries exactly one GTP command id, and the ids strictly increase.
        // Sending `4 4 kata-analyze ...` would be rejected by the engine, so this is a real check.
        let parsed: Vec<(u64, String)> = lines
            .iter()
            .map(|line| split_live_command(line).unwrap_or_else(|| panic!("no command id in {line:?}")))
            .collect();
        for pair in parsed.windows(2) {
            assert!(
                pair[1].0 > pair[0].0,
                "command ids must strictly increase: {parsed:?}"
            );
        }

        let (analyze_id, analyze) = parsed
            .iter()
            .find(|(_, body)| body.starts_with("kata-analyze"))
            .expect("the engine must receive a kata-analyze command");
        assert!(
            analyze.contains(" ownership true"),
            "ownership must be requested or the heatmap is permanently empty: {analyze}"
        );
        assert!(
            analyze.contains(" rootInfo true"),
            "rootInfo must be requested or the frame silently degrades: {analyze}"
        );
        // The acknowledgement that opens the stream belongs to the last command, so `kata-analyze`
        // must own the highest id of the batch.
        assert_eq!(
            *analyze_id,
            parsed.last().map(|(id, _)| *id).unwrap(),
            "kata-analyze must be the last command sent: {parsed:?}"
        );

        // Order matters: boardsize, komi, clear_board, then the moves, then the analysis command.
        let bodies: Vec<&str> = parsed.iter().map(|(_, body)| body.as_str()).collect();
        assert_eq!(bodies.first().copied(), Some("boardsize 9"));
        assert!(bodies.contains(&"komi 7.5"), "komi must be forwarded: {bodies:?}");
        let index_of = |needle: &str| bodies.iter().position(|body| *body == needle);
        let clear_index = index_of("clear_board").expect("clear_board");
        let first_move = index_of("play B E5").expect("play B E5");
        let second_move = index_of("play W E3").expect("play W E3");
        assert!(
            clear_index < first_move && first_move < second_move,
            "order wrong: {bodies:?}"
        );
        assert_eq!(bodies.last().copied(), Some(analyze.as_str()));
    }

    /// When no `komi` is supplied the command must be omitted entirely, so the engine keeps its
    /// configured default rather than being overwritten with a wrong value.
    #[cfg(unix)]
    #[test]
    fn live_review_position_omits_komi_when_absent() {
        let lines = echoed_live_review_lines("live-review-nokomi", 19, None, Vec::new(), None, 10);

        let bodies: Vec<String> = lines
            .iter()
            .map(|line| {
                split_live_command(line)
                    .unwrap_or_else(|| panic!("no command id in {line:?}"))
                    .1
            })
            .collect();
        assert!(
            !bodies.iter().any(|body| body.starts_with("komi")),
            "an absent komi must not be sent: {bodies:?}"
        );
        assert!(
            bodies
                .iter()
                .any(|body| body == "kata-analyze 10 ownership true rootInfo true"),
            "{bodies:?}"
        );
    }

    // --- Real-engine live-review branch test ---------------------------------------------
    //
    // The fake-engine tests above prove the *commands* are right. They cannot prove that a real
    // KataGo ends up with the right *stones* after a same-turn branch switch: the defect this test
    // targets (`set_free_handicap` after an implicit board carry-over -> `Board is not empty`) is
    // invisible to an engine that echoes instead of playing.
    //
    // Opt-in, like the `crates/engine-manager` real-engine tests: skipped unless the three paths
    // below are exported, so `cargo test` stays green without KataGo installed.

    /// The real KataGo paths for the live-review branch test, or `None` when not configured.
    ///
    /// Every path comes from the environment. Nothing here hardcodes a user directory, and the
    /// function returns `None` (not a panic) when a path is absent or does not exist, so an
    /// unconfigured machine skips instead of failing.
    #[cfg(unix)]
    fn real_live_review_profile(name: &str) -> Option<(EngineProfileDto, PathBuf)> {
        let engine = std::env::var("LIZZIEYZY_REVIEW_BRANCH_KATAGO_ENGINE").ok()?;
        let model = std::env::var("LIZZIEYZY_REVIEW_BRANCH_KATAGO_MODEL").ok()?;
        let config = std::env::var("LIZZIEYZY_REVIEW_BRANCH_KATAGO_CONFIG").ok()?;
        if !Path::new(&engine).exists() || !Path::new(&model).exists() || !Path::new(&config).exists() {
            return None;
        }
        // A private working directory: the bundled GTP config's `logDir` is relative, so running
        // with the repository as cwd would drop engine logs into the working tree.
        let working_dir = native_config_temp_dir(name);
        fs::create_dir_all(&working_dir).unwrap();
        Some((
            EngineProfileDto {
                name: format!("real katago ({name})"),
                engine_path: engine,
    ...
    /// Parse one row of a `showboard` grid into `(GTP vertex, colour)` stones.
    ///
    /// KataGo v1.18.2 prints the grid as a column header, then one line per row (captured verbatim
    /// from the engine, not reconstructed):
    ///
    /// ```text
    ///    A B C D E F G H J
    ///  9 . . . . . . . . .
    ...
    /// ```
    ///
    /// Three details this parser has to get right, all of which an earlier version got wrong by
    /// splitting on whitespace instead of on fixed columns:
    ///
    /// * The first three characters are the label — a space-padded row number (`" 9"`, `"19"`).
    /// * The cells then start immediately at offset 3, one character each, separated by single
    ///   spaces. Column `n` is therefore at offset `3 + 2 * n`, and a full row spans
    ///   `3 + 2 * board_size - 1` characters: the trailing separator is absent after the last cell.
    ///   `split_whitespace` instead yields one single-character "cell" followed by glued `O1`/`X2`
    ///   cells, which either drops a stone or attributes it to the wrong column.
    /// * A cell may be `X`, `O`, `.`, or a stone with the move number appended (`X2`, `O1`, `X10`)
    ///   — KataGo annotates the last move that way. Only that one character decides occupancy,
    ///   which is why the exact separator layout matters.
    ///
    /// GTP spells columns with `A..H`, `J..T` — `I` is skipped — so column `n` is the 0-based column
    /// of that letter sequence. Column 9 is therefore `J`, not `I`.
    ///
    /// Returns `None` when the line is not a row of a `board_size` board, which is how the column
    /// header, the blank lines and the trailing legend are skipped. A row of the right width with no
    /// stones still returns an empty vector, which is what makes this work on an empty board.
    #[cfg(unix)]
    fn showboard_row_stones(board_size: u8, line: &str) -> Option<Vec<(String, char)>> {
        /// The GTP column letters, `I` excluded.
        const GTP_COLUMNS: &str = "ABCDEFGHJKLMNOPQRSTUVWXYZ";
        /// The label column: a space-padded row number, then the separator before the first cell.
        const CELLS_START: usize = 3;
        /// One cell plus its trailing separator.
        const CELL_STRIDE: usize = 2;

        let cells = line.get(CELLS_START..)?;
        // The last cell has no trailing separator, so a full row is one character short of the
        // stride. This is also what rejects the file labels and the legend.
        if cells.len() != CELL_STRIDE * board_size as usize - 1 {
            return None;
        }
        // The label must be checked for *digits* before parsing: `"   A"` for the column header is
        // also three characters, and `u8::from_str` accepts non-decimal notation, so parsing `"A"`
        // as a `u8` succeeds with 10. That single oversight made the column header pass as a row
        // whose "stones" were the nine letters `B`..`J`, which is worse than rejecting it.
        let label = line.get(..CELLS_START)?;
        let digits = label.trim();
        if digits.is_empty() || !digits.bytes().all(|byte| byte.is_ascii_digit()) {
            return None;
        }
        let line_row: u8 = digits.parse().ok()?;
        if line_row == 0 || line_row > board_size {
            return None;
        }

        let mut stones = Vec::new();
        for column in 0..board_size as usize {
            let cell = cells.as_bytes().get(CELL_STRIDE * column)?;
            let column_letter = GTP_COLUMNS.get(column..=column)?;
            // `X` is Black and `O` is White in `showboard`'s legend. `X2`/`O1` are the same stones
            // with their move number in the following cell, so only this byte is read; every other
            // mark (`.` for empty, `+` for the last move) is not a stone.
            let color = match cell {
                b'X' | b'x' => 'B',
                b'O' | b'o' => 'W',
                _ => continue,
            };
            // The label is already validated as a non-zero row of this board, so the vertex is the
            // column letter followed by that row — e.g. `C7`.
            let vertex = format!("{column_letter}{line_row}");
            stones.push((vertex, color));
        }
        Some(stones)
    }

    /// Pins [`showboard_row_stones`] to bytes captured from a real KataGo v1.18.2 `showboard`.
    ///
    /// This is the cheap, GPU-free guard for the parser: the real-engine test below is opt-in, so
    /// without this a regression in the row layout would only surface on a machine with KataGo
    /// installed. The sample is the exact response for `set_free_handicap C7 G3` + `W G7` +
    /// `B C3` + `W E5` on 9x9, whose five stones are `B C7 G3 C3` and `W G7 E5`.
    #[cfg(unix)]
    #[test]
    fn showboard_row_parser_reads_the_real_kata_go_grid() {
        // Verbatim, including the single leading space in ` 9` and the missing separator before the
        // move-number annotation in `O1.` / `X2.`.
        let board = concat!(
            "   A B C D E F G H J\n",
            " 9 . . . . . . . . .\n",
            " 8 . . . . . . . . .\n",
        ...
        for line in board.lines() {
            let Some(stones) = showboard_row_stones(9, line) else {
                continue;
            };
        ...
        // `X2` is Black C3 and `O1` is White G7 — the move-number annotation must not hide either
        // stone, and must not shift the column it is attributed to.
        assert_eq!(black, vec!["C3", "C7", "G3"]);
        assert_eq!(white, vec!["E5", "G7"]);

        // The column header and the legend are not rows: a header has no numeric label, and the
        // legend's length does not match the board.
        //
        // The header is the interesting case — it is exactly as wide as a row, so only the label
        // check rejects it, and `u8::from_str` would happily read its `A` as 10 if the label were
        // not first required to be digits. A header accepted as a row reports the nine column
        // letters as stones.
        assert!(showboard_row_stones(9, "   A B C D E F G H J").is_none());
        assert!(showboard_row_stones(9, "Next player: Black").is_none());
        assert!(showboard_row_stones(9, "Rules: {\"friendlyPassOk\":true}").is_none());
        assert!(showboard_row_stones(9, "").is_none());
        // An empty row is a valid row with no stones, not a parse failure.
        assert_eq!(showboard_row_stones(9, " 9 . . . . . . . . ."), Some(vec![]));
        // A 19x19 row must not be read as a 9x9 one.
        assert!(showboard_row_stones(9, "19 . . . . . . . . . . . . . . . . . . .").is_none());
        assert!(showboard_row_stones(19, "19 . . . . . . . . . . . . . . . . . . .").is_some());
        // Column 9 is `J` — GTP skips `I`.
        let row_with_j = showboard_row_stones(9, " 9 . . . . . . . . X").unwrap();
        assert_eq!(
            row_with_j.last().map(|(v, c)| (v.as_str(), *c)),
            Some(("J9", 'B'))
        );
    }

    /// The exact five-stone occupancy that every branch of the 9x9 handicap acceptance game must
    /// report, as a set of `(GTP vertex, colour)` stones.
    ///
    /// The game is `docs/qa/codex-live-review-evidence/handicap-branches-9x9.sgf`:
    ///
    /// ```text
    /// HA[2] AB[cc][gg] PL[W] ;W[gc] ;B[cg] ( ;W[ee] )( ;W[fe] )
    ///        C7    G3            G7     C3      E5        F5
    /// ```
    ///
    /// The two variations differ only in the vertex of the move after `B C3`, which is
    /// `W E5` (mainline) or `W F5` (the same-turn alternative). Note the colour: **the last move is
    /// White**, so the extra stone is White either way and the board carries no Black `E5`/`F5` at
    /// all. A "branch" is therefore the branch node plus the one move that leaves it, i.e.
    /// `[W,G7, B,C3, W,E5|F5]` on top of the fixed `setup=[B,C7, B,G3], handicap=2`.
    #[cfg(unix)]
    fn acceptance_showboard_stones(extra: &str) -> Vec<(String, char)> {
    ...
    /// Send `showboard` on the *existing* session and return the board it reports.
    ///
    /// This is the point of the test: it asks the engine that is already analysing, so the answer
    /// describes the board the analysis is actually running on — not a fresh session that was
    /// started from the same commands and might agree only if both were wrong in the same way.
    ///
    /// Stone occupancy is read from the `X`/`O` grid. Ownership is deliberately **not** used: it is
    /// a probabilistic heatmap, so a single vertex's sign is not evidence that a stone is present.
    ///
    /// `next_command_id` is advanced by this call. The id must be the *next unused* one: KataGo
    /// suppresses a command whose id is not greater than the last one it saw, answering with only
    /// `= <id> <board hash>`. Re-using an id therefore looks like a board with no stones rather
    /// than like an error, which is exactly the trap an earlier version of this helper fell into.
    #[cfg(unix)]
    fn showboard_stones(
        session: &mut engine_manager::GtpSession,
        board_size: u8,
        ...
        *next_command_id = next_command_id.saturating_add(1);
        ...
        // A fresh request stops the running `kata-analyze`, so the acknowledgements below are
        // unambiguous: only this request's response is in flight.
        session
            .send_command(&format!("{command_id} showboard"))
            .expect("showboard must be accepted by the session");

        // The response is complete once all `board_size` rows have arrived. KataGo answers
        // `= <id> MoveNum: 0 HASH: ...`, then one line per board row, then the
        // `Next player:`/`Rules:`/captured-stones legend. So the board rows are exactly the lines
        // that parse as rows of this board, and waiting for *all* of them is what makes the read
        // reliable. Both halves matter:
        //
        // * Collecting is keyed on the *body* of each response line, **not** on
        //   [`split_live_command`]: only the `=` acknowledgement starts with `<id> `, every board row
        //   after it is plain text, and a `split_live_command` filter discards the entire grid,
        //   leaving an empty response.
        // * The loop must wait for the last row, not stop at the first one. Stopping early yields a
        //   single row, and since the grid runs top-down, an early row is exactly where a stone is
        //   least likely to be — so the read looks like "the board is empty" instead of "the read
        //   was truncated".
        let mut lines: Vec<String> = Vec::new();
        let deadline = std::time::Instant::now() + Duration::from_secs(30);
        let board_rows = loop {
            let rows: Vec<Vec<(String, char)>> = lines
                .iter()
                .filter_map(|line| showboard_row_stones(board_size, line))
                .collect();
            if rows.len() == board_size as usize {
                break rows;
            }
            if std::time::Instant::now() >= deadline {
                panic!("showboard returned no board grid within 30s: {lines:?}");
            }
            match session.next_event_timeout(Duration::from_millis(200)) {
                Some(engine_manager::GtpSessionEvent::Line(line)) => {
                    let trimmed = line.trim_end();
                    if trimmed.starts_with('?') {
                        panic!("showboard was rejected by the engine: {line:?}");
                    }
                    // The echoed request, seen while the previous `kata-analyze` was still flushing.
                    if trimmed.contains("showboard") {
                        continue;
                    }
                    lines.push(line);
                }
                Some(engine_manager::GtpSessionEvent::Eof { exit_code }) => {
                    panic!("the engine exited during showboard (exit code {exit_code:?})")
                }
                Some(engine_manager::GtpSessionEvent::ReadError(error)) => {
                    panic!("the engine read failed during showboard: {error}")
                }
                None => {}
            }
        };
    ...
    /// Drive the **production** live-review position path against a real KataGo and read the board.
    ///
    /// This is the regression test for "the previous branch's stone is still on the board". It
    /// replays the acceptance sequence on 9x9: the `W E5` branch, then the `W F5` branch at the
    /// same move number, then back to `W E5`, and after each one asks the same session for
    /// `showboard`.
    ///
    /// Why `showboard` and not ownership: the earlier QA run judged the third `W E5` by its
    /// ownership value (−0.257, short of the −0.4 it was compared against) and treated that as a
    /// missing stone. Ownership at one vertex is a probability, so it cannot decide occupancy in
    /// either direction — that criterion was unreliable and no conclusion about a lost stone
    /// follows from it. `showboard` reports the actual position the engine is searching.
    ///
    /// The request mirrors [`buildLivePosition`] in `apps/desktop/src/domain/livePosition.ts`: the
    /// SGF's `AB` stones go into `setup` **with their real Black colour** (they must not be written
    /// as White), `handicap` is the `HA[2]`, and `player` is the node's `to_play`. The handicap
    /// stones are then installed by `set_free_handicap` from that setup.
    ///
    /// The invariant, identical for all three branches: the board holds **exactly five stones** —
    /// Black `C7`, `G3`, `C3`; White `G7`; and White `E5` or `F5`, whichever the branch played. The
    /// other variation's vertex must be empty, which is the stale-stone check in both directions.
    #[cfg(unix)]
    #[test]
    fn real_engine_live_review_same_turn_branch_switch_replaces_the_variation_stone() {
        let Some((profile, working_dir)) = real_live_review_profile("real-review-branch") else {
            eprintln!(
                "skipping real-engine branch test: LIZZIEYZY_REVIEW_BRANCH_KATAGO_ENGINE / \
                 _MODEL / _CONFIG not set to existing paths"
            );
            return;
        };
        let spec = build_command_spec(&profile).expect("a real KataGo profile must build a spec");
        let mut session = engine_manager::GtpSession::start(&spec).expect("the real engine must start");
        // The real route: no Tauri app, no IPC, no registry. `apply_live_review_position` is the
        // same function `run_live_review_owner` calls for every `katago_live_review_set_position`.
        //
        // `setup` carries the two `AB` stones as Black, and `handicap` is the SGF's `HA[2]`, so the
        // production path installs them with `set_free_handicap` and then resets the board for each
        // branch. Passing White tokens here (or a non-Black colour for a handicap stone) is the bug
        // this test exists to catch, so the colours are the SGF's own.
        let mut command_id: u64 = 1;
        // `command_id` is deliberately *not* borrowed by this closure: capturing it by value would
        // let the ids inside the batch advance a copy while the caller's `command_id` stayed at 1,
        // and the following `showboard` would then reuse an id KataGo has already seen and silently
        // suppress. Passing the current id in and taking the advanced one back keeps one counter.
        let apply = |session: &mut engine_manager::GtpSession,
                     moves: Vec<String>,
                     player: &str,
                     mut command_id: u64|
         -> u64 {
            let request = LiveReviewPositionRequest {
                board_size: 9,
                setup: vec!["B".into(), "C7".into(), "B".into(), "G3".into()],
                moves,
                rules: Some("chinese".into()),
                handicap: Some(2),
                komi: Some(0.5),
            ...
            apply_live_review_position(session, &request, 10, &mut command_id)
                .expect("applying a real live-review position must succeed");
            command_id
        };

        // The two branches differ only in the last move: `W E5` (mainline) or `W F5` (the same-turn
        // alternative). Both replay the whole game, because that is what the production path does
        // when the user selects a node: `clear_board`, the handicap stones, then every move up to
        // and including the selected node.
        let branch_e5: Vec<String> = ["W", "G7", "B", "C3", "W", "E5"]
            .iter()
            .map(|token| token.to_string())
            .collect();
        let branch_f5: Vec<String> = ["W", "G7", "B", "C3", "W", "F5"]
            .iter()
            .map(|token| token.to_string())
            .collect();
        assert_ne!(branch_e5, branch_f5, "the two branches must differ");

        // The handicap stones really are installed as a handicap (HA=2 -> `set_free_handicap`), from
        // the setup's own Black tokens.
        assert_eq!(
            live_review_handicap_stones(&LiveReviewPositionRequest {
                board_size: 9,
                setup: vec!["B".into(), "C7".into(), "B".into(), "G3".into()],
                moves: Vec::new(),
                rules: None,
        ...
        for (label, moves, extra) in [
            ("W E5 (mainline)", branch_e5.clone(), "E5"),
            ("W F5 (same-turn variation)", branch_f5.clone(), "F5"),
            ("W E5 (back to the mainline)", branch_e5.clone(), "E5"),
        ] {
            // After `W E5`/`W F5` it is Black to move: `position.to_play` of the selected node.
            command_id = apply(&mut session, moves, "black", command_id);
            // `showboard_stones` takes the next unused id and advances `command_id` itself, so the
            // id can never collide with one the batch already used.
            let (black, white) = showboard_stones(&mut session, 9, &mut command_id);

            // The whole position, not just "the stones we cared to look for": a five-stone set
            // equality fails on a leftover from the previous branch *and* on an unexpected extra.
            let mut reported: Vec<(String, char)> = black
                .iter()
                .map(|vertex| (vertex.clone(), 'B'))
                .chain(white.iter().map(|vertex| (vertex.clone(), 'W')))
                .collect();
            reported.sort();
            let expected = acceptance_showboard_stones(extra);
            assert_eq!(
                reported, expected,
                "[{label}] the board is not the requested position: \
                 black{black:?} white{white:?}"
            );

            // The other variation's vertex must be empty, whichever branch this is — that is the
            // stale-stone check, and it is symmetric so neither direction can hide behind the other.
            let stale = if extra == "E5" { "F5" } else { "E5" };
            assert!(
                !black.contains(&stale.to_string()) && !white.contains(&stale.to_string()),
                "[{label}] {stale} survived from the previous branch: the board was not reset, so \
                 the engine is analysing a merged position. B{black:?} W{white:?}"
            );

            observed.push((label, black, white));
        }

        // The branches must not all report the same board, which is what makes the checks above
        // meaningful rather than three copies of one observation.
        assert_ne!(
            observed[0].2, observed[1].2,
            "the W E5 and W F5 branches reported identical White stones; the switch did nothing"
        );
        assert_eq!(
            observed[0].2, observed[2].2,
            "returning to the mainline must report the mainline's White stones"
        );

        // The engine must still own the same process: this is one session, not one engine per move.
        assert_eq!(
            session.close().expect("the engine must close cleanly"),
            Some(0),
            "the engine did not exit cleanly"
        );
        let _ = fs::remove_dir_all(&working_dir);
    }

    /// Regression test for the non-blocking requirement (plans/003 defect B).
    ///
    /// `katago_analyze_once` used to be a synchronous command, so a long analysis ran inline on the
    /// IPC handler and froze the whole app. It is now `async fn` + `spawn_blocking`.
    ///
    /// The test must distinguish "defers the blocking work" from "runs it inline", and a plain
    /// concurrent-ticker test cannot: Tauri's runtime is multi-threaded, so a second worker keeps
    /// ticking even while one future blocks inline. Instead this drives the returned future by
    /// hand and measures how long a single `poll` occupies the calling thread. A `spawn_blocking`
    /// implementation returns `Pending` almost immediately; an inline implementation does the whole
    /// engine run inside that first `poll`.
    #[cfg(unix)]
    #[test]
    fn katago_analyze_once_poll_returns_promptly_instead_of_running_engine_inline() {
        use std::future::Future;
        use std::task::{Context, Poll, Waker};
        use std::time::Instant;

        let dir = native_config_temp_dir("nonblocking-poll-once");
        // The fake engine sleeps well over a second before answering.
        let (profile, sgf) = fake_gtp_profile_for_sgf(
            &dir,
            r#"read line; sleep 3; printf '{"id":"x","turnNumber":0,"rootInfo":{"visits":1,"winrate":0.5,"scoreMean":0.0}}\n'"#,
            "(;GM[1]FF[4]SZ[19]KM[7.5];B[pd];W[dd])",
        );

        let mut future = Box::pin(analyze_once_frame(profile, sgf, 0, 8, EngineSlotGuard::inert()));
        ...
        // The engine needs ~3s. A single poll must not sit there waiting for it.
        assert!(
            elapsed < Duration::from_secs(1),
            "first poll blocked the caller for {elapsed:?}; the engine work is running inline \
             instead of on a blocking thread"
        );

        // The work is genuinely still in flight, i.e. it was deferred rather than short-circuited.
        if matches!(first_poll, Poll::Ready(_)) {
            panic!(
                "analysis finished inside the first poll; the fake engine was expected to still be running"
            );
        }

        // And it does complete when driven to the end.
        let frame = tauri::async_runtime::block_on(future).expect("analysis should succeed");
        assert_eq!(frame.turn, 0);

        let _ = fs::remove_dir_all(&dir);
    }

    /// Same property for the batch command, which can block for up to 600s.
    #[cfg(unix)]
    #[test]
    fn katago_analyze_game_poll_returns_promptly_instead_of_running_engine_inline() {
        use std::future::Future;
        use std::task::{Context, Poll, Waker};
        use std::time::Instant;

        let dir = native_config_temp_dir("nonblocking-poll-game");
        let (profile, sgf) = fake_gtp_profile_for_sgf(
            &dir,
            r#"read line; sleep 3; printf '{"id":"x","turnNumber":0,"rootInfo":{"visits":1,"winrate":0.5,"scoreMean":0.0}}\n{"id":"x","turnNumber":1,"rootInfo":{"visits":1,"winrate":0.5,"scoreMean":0.0}}\n'"#,
            "(;GM[1]FF[4]SZ[19]KM[7.5];B[pd])",
        );

        let mut future = Box::pin(analyze_game_frames(profile, sgf, 8, EngineSlotGuard::inert()));
        let waker = Waker::noop();
        let mut cx = Context::from_waker(waker);
        ...
        if matches!(first_poll, Poll::Ready(_)) {
            panic!("batch analysis finished inside the first poll; the fake engine was expected to still be running");
        }

        let frames = tauri::async_runtime::block_on(future).expect("batch analysis should succeed");
        assert_eq!(frames.len(), 2);

        let _ = fs::remove_dir_all(&dir);
    }

    /// The GTP command spec must carry the weights and a config, and the config must be a GTP
    /// config rather than the batch `analysis.cfg` (plans/GTP_LIVE_PROBE_FINDINGS.md §1).
    #[cfg(unix)]
    #[test]
    fn katago_gtp_profile_builds_spec_with_model_and_config() {
        let dir = native_config_temp_dir("gtp-spec");
        let profile = fake_gtp_profile(&dir, "exit 0");

        let spec = build_command_spec(&profile).unwrap();

        assert_eq!(spec.args[0], "gtp");
        assert!(spec.args.iter().any(|arg| arg == "-model"));
        assert!(spec.args.iter().any(|arg| arg == "-config"));
        // Locate the config by the `-config` flag rather than by position: the spec also carries
        // `-override-config reportAnalysisWinratesAs=BLACK` after it, so `args.last()` is no longer
        // the config path.
        let config_flag = spec
            .args
            .iter()
            .position(|arg| arg == "-config")
            .expect("the GTP spec must pass -config");
        let config = &spec.args[config_flag + 1];
        assert!(
            config.ends_with("gtp.cfg"),
            "live review must use a GTP config, got {config}"
        );
        assert!(
            !config.ends_with("analysis.cfg"),
            "the batch analysis config must not be used for GTP"
        );
        ...
        let perspective_flag = spec
            .args
            .iter()
            .position(|arg| arg == "-override-config")
            .expect("the GTP spec must pin the reporting perspective");
        assert_eq!(
            spec.args[perspective_flag + 1],
            "reportAnalysisWinratesAs=BLACK",
            "the GTP engine must report winrate/score/ownership from Black's perspective"
        );
    ...
    #[test]
    fn accepts_live_info_line_rejects_stale_generation() {
        // A line tagged with generation 1 must be dropped once generation 2 is current: this is
        // the frame from move A arriving after the user already jumped to move B.
        assert!(!accepts_live_info_line(2, 1, Some(7), Some(7), true));
    }

    #[test]
    fn accepts_live_info_line_rejects_stale_turn() {
        // Same generation but a different target turn is a superseded request (set_position bumps
        // both, but keeping the two dimensions independent is what makes the guard explicit).
        assert!(!accepts_live_info_line(3, 3, Some(7), Some(5), true));
    }

    #[test]
    fn accepts_live_info_line_accepts_matching_generation_and_turn() {
        assert!(accepts_live_info_line(3, 3, Some(7), Some(7), true));
    }

    #[test]
    fn accepts_live_info_line_rejects_when_no_target() {
        // After `stop` there is no target turn, so no frame may be rendered.
        assert!(!accepts_live_info_line(4, 4, None, Some(7), true));
    }

    #[test]
    fn live_review_set_position_bumps_generation() {
        // Keep the receiver alive: a dropped receiver makes the send fail with "session has ended",
        // which is a different failure than the one under test.
        let (sender, _receiver) = std::sync::mpsc::channel();
        let state = Mutex::new(LiveReviewSession {
            commands: Some(sender),
            ..LiveReviewSession::default()
        });

        let plain = |moves: Vec<String>| LiveReviewPositionRequest {
            board_size: 19,
            setup: Vec::new(),
            moves,
            rules: None,
        ...
        let first = set_live_review_position(&state, plain(vec!["B".into(), "D4".into()]), 1, 10).unwrap();
        let second = set_live_review_position(&state, plain(vec!["B".into(), "D4".into()]), 2, 10).unwrap();

        assert!(second > first, "generation must increase monotonically");
        let live = state.lock().unwrap();
        assert_eq!(live.generation, second);
        assert_eq!(live.target_turn, Some(2));
        // The shared guard must observe the newest generation, otherwise in-flight lines from the
        // first request would still be accepted.
        assert_eq!(guard_snapshot(&live.guard).0, second);
    }

    #[test]
    fn live_review_default_interval_is_ten_centisec() {
        assert_eq!(normalize_live_review_interval(0), 10);
        assert_eq!(normalize_live_review_interval(25), 25);
    }

    #[test]
    fn live_review_kata_analyze_command_always_requests_ownership_and_root_info() {
        // Both flags are silent-failure switches: without `ownership true` the heatmap is empty,
        // without `rootInfo true` the frame degrades to best-move statistics.
        for command in [
            live_review_kata_analyze_command(Some("B"), 10),
            live_review_kata_analyze_command(None, 10),
        ] {
            assert!(
                command.contains(" ownership true"),
                "missing ownership: {command}"
            );
            assert!(command.contains(" rootInfo true"), "missing rootInfo: {command}");
            assert!(command.contains("10"), "missing interval: {command}");
        }
        assert!(live_review_kata_analyze_command(Some("W"), 20).starts_with("kata-analyze W 20"));
    }

    #[test]
    fn live_review_stop_tears_the_session_down_and_clears_its_slot() {
        // The old `stop` only paused the analysis, leaving the engine, the registry entry and the
        // exclusive slot alive — so a later batch start was rejected for a session the UI considered
        // stopped. Stopping must now actually end the session.
        let registry = AnalysisJobRegistry::default();
        let _slot = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-1".to_string())
            .unwrap();
        let live_done = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
        let owner_done = std::sync::Arc::clone(&live_done);
        let owner = std::thread::spawn(move || {
            owner_done.store(true, std::sync::atomic::Ordering::SeqCst);
        });
        let (sender, receiver) = std::sync::mpsc::channel();
        let state = Mutex::new(LiveReviewSession {
            commands: Some(sender),
            forwarder: Some(owner),
            job_id: Some("live-1".to_string()),
            ..LiveReviewSession::default()
        });

        stop_live_review(&state, &registry).unwrap();

        // The owner was signalled, joined, and the slot is free again for the next engine session.
        assert!(matches!(receiver.try_recv(), Ok(LiveReviewCommand::Shutdown)));
        assert!(live_done.load(std::sync::atomic::Ordering::SeqCst));
        assert!(!registry.contains("live-1"));
        assert_eq!(registry.kind_of_first(), None);
        assert!(!state.lock().unwrap().has_session());
        // A fresh exclusive slot can now be taken, which is what "stopped" has to mean.
        assert!(registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "batch-after-stop".to_string())
            .is_ok());
    }

    /// Integration-level staleness proof with the real handler state (not just the pure function):
    /// a line belonging to move A must be rejected once move B's analysis has been requested.
    ///
    /// This mirrors the real race — the owner thread snapshots the generation when it reads a line,
    /// the user jumps to another move, and the line is re-checked against the *current* guard before
    /// being emitted. If the guard is ever weakened to "generation is not in the future" or similar,
    /// this test fails.
    #[test]
    fn live_review_drops_in_flight_lines_after_a_new_request() {
        let (sender, _receiver) = std::sync::mpsc::channel();
        let state = Mutex::new(LiveReviewSession {
            commands: Some(sender),
            ..LiveReviewSession::default()
        ...
        // First request for turn 5: the owner thread reads a line and snapshots its metadata.
        set_live_review_position(
            &state,
            LiveReviewPositionRequest {
                board_size: 19,
                setup: Vec::new(),
        ...
        .unwrap();
        let (line_generation, line_target) = {
            let live = state.lock().unwrap();
            (live.generation, live.target_turn)
        };
        assert!(
            accepts_live_info_line(line_generation, line_generation, line_target, line_target, true),
            "the current request's own line must pass the guard"
        );

        // A newer request for the *same* turn arrives while that line is still in flight. Both
        // requests target turn 5, so only the generation distinguishes them: this is the case a
        // turn-only guard cannot catch (re-requesting the same move), and it is why the guard
        // checks generation *and* turn.
        let newer_generation = set_live_review_position(
            &state,
            LiveReviewPositionRequest {
                board_size: 19,
                setup: Vec::new(),
        ...
        .unwrap();
        assert!(newer_generation > line_generation);

        let live = state.lock().unwrap();
        let (current_generation, current_target_turn, _, stream_ready) = guard_snapshot(&live.guard);
        assert!(
            !accepts_live_info_line(
                current_generation,
                line_generation,
                current_target_turn,
                line_target,
            ...
            "a frame from the superseded request must not pass the guard even though the turn matches"
        );
    }

    /// Cancelling signals the token but must **not** free the exclusive slot.
    ///
    /// A cancelled batch worker can still be tearing its engine down; reopening the slot at cancel
    /// time would let a new job start on top of a dying one. The slot is released by the worker's own
    /// `remove_if_owner` when it really returns, which is what the second half of this test asserts.
    #[test]
    fn katago_cancel_registry_signals_the_token_but_keeps_the_slot_until_the_worker_finishes() {
        let registry = AnalysisJobRegistry::default();
        let slot = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-1".to_string())
            .unwrap();
        ...
        // Idempotent and observable on the token the worker actually polls.
        assert!(first_cancel);
        assert!(second_cancel);
        assert!(slot.cancel_token.is_cancelled());
        // The slot survives the cancel, so no second job can start while the engine is still exiting.
        assert!(registry.contains("job-1"));
        assert!(registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-2".to_string())
            .is_err());

        // The worker's completion is what releases it — and it is reusable immediately afterwards,
        // which is the guarantee the frontend's "restart after cancel" depends on.
        assert!(registry.remove_if_owner("job-1", &slot));
        assert!(!registry.contains("job-1"));
        let replacement = registry.try_insert_exclusive(AnalysisJobKind::Batch, "job-2".to_string());
        assert!(
            replacement.is_ok(),
            "the slot must be reusable once the job has finished"
        );
    }

    /// Releasing the slot twice (once on the cancel path, once on the common epilogue) is a no-op.
    #[test]
    fn registry_slot_release_is_idempotent() {
        let registry = AnalysisJobRegistry::default();
        let slot = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-1".to_string())
            .unwrap();
        ...
        assert!(registry.remove_if_owner("job-1", &slot));
        // Second release: the entry is gone, so this must report "not mine" rather than panic or
        // disturb whatever took the slot in the meantime.
        assert!(!registry.remove_if_owner("job-1", &slot));
        ...
        assert!(!registry.remove_if_owner("job-1", &slot));
        assert!(registry.contains("live-1"));
        assert!(slot_still_owns(&registry, "live-1", &successor));
    }

    /// A finishing job must not evict a slot that a newer job has already taken.
    ///
    /// The token is the entry's identity, so this proves the release is exact: an old worker holding
    /// a stale slot cannot remove whichever entry happens to sit at its key. The successor is put at
    /// the *same* key on purpose — that is the only case where "remove by key alone" would be wrong,
    /// and it is guaranteed never to happen in production because every job mints a fresh Uuid. The
    /// invariant is therefore asserted directly against the registry rather than by pretending a
    /// real job id gets reused.
    #[test]
    fn registry_remove_if_owner_refuses_to_evict_a_successor() {
        let registry = AnalysisJobRegistry::default();
        let stale = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-1".to_string())
            .unwrap();
        assert!(registry.remove_if_owner("live-1", &stale));

        // A different entry (different token) now occupies the same key.
        let successor_token = AnalysisCancelToken::new();
        registry.jobs.lock().unwrap().insert(
            "live-1".to_string(),
            AnalysisJobEntry {
                kind: AnalysisJobKind::Live,
                cancel_token: successor_token.clone(),
            },
        );

        // The stale worker's release must be refused: the new token is not its own.
        assert!(!registry.remove_if_owner("live-1", &stale));
        assert!(registry.contains("live-1"));

        // And the successor's own release still works, which is what keeps the slot from leaking.
        let successor = AnalysisJobSlot {
            cancel_token: successor_token,
        };
        assert!(registry.remove_if_owner("live-1", &successor));
        assert!(!registry.contains("live-1"));
    }

    /// Whether `slot` is still the registered entry for `job_id`.
    fn slot_still_owns(registry: &AnalysisJobRegistry, job_id: &str, slot: &AnalysisJobSlot) -> bool {
        registry
            .jobs
            .lock()
            .map(|jobs| {
                jobs.get(job_id)
                    .is_some_and(|entry| entry.cancel_token.is_same_as(&slot.cancel_token))
            })
            .unwrap_or(false)
    }

    /// The exclusive slot is a single slot even when the kinds differ.
    #[test]
    fn registry_exclusive_slot_rejects_a_different_kind_and_reports_the_owner() {
        let registry = AnalysisJobRegistry::default();
        let _live = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-1".to_string())
            .unwrap();
        ...
        let error = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "batch-1".to_string())
            .unwrap_err();

        assert!(error.contains("batch analysis"), "{error}");
        assert!(
            error.contains("live review"),
            "the error must name the owner: {error}"
        );
        assert_eq!(registry.kind_of_first(), Some(AnalysisJobKind::Live));
        assert!(!registry.contains("batch-1"));
    }

    /// A single-shot command must *take* the slot, not merely observe that it is empty.
    ///
    /// The old check-only form was not mutual exclusion: two concurrent `analyze_once` calls would
    /// each see an empty slot before either registered anything, and both would spawn an engine. The
    /// second claim below is the regression.
    #[test]
    fn single_shot_engine_starts_claim_the_slot_and_are_refused_while_it_is_held() {
        let registry = AnalysisJobRegistry::default();

        // First single-shot run: claims the slot.
        let (first_job_id, first_slot) = claim_single_shot_slot(&registry, AnalysisJobKind::Batch).unwrap();
        assert!(registry.contains(&first_job_id));

        // A second single-shot run arriving before the first finished must be refused, because the
        // first now *holds* the slot rather than having merely looked at it.
        let error = claim_single_shot_slot(&registry, AnalysisJobKind::Batch).unwrap_err();
        assert!(error.contains("batch analysis"), "{error}");

        // Releasing the first (the guard's job) reopens the slot.
        assert!(registry.remove_if_owner(&first_job_id, &first_slot));
        assert!(claim_single_shot_slot(&registry, AnalysisJobKind::Batch).is_ok());

        // A registered live session blocks a single-shot run, and names itself in the error.
        let registry = AnalysisJobRegistry::default();
        let _live = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-1".to_string())
            .unwrap();
        ...
        let error = claim_single_shot_slot(&registry, AnalysisJobKind::Batch).unwrap_err();
        assert!(error.contains("live review"), "{error}");
    }

    /// A running live session whose owner thread is still returning, as a start's teardown sees it.
    ///
    /// The owner releases *its own* registry entry as it returns — before `join_live_review_owner`
    /// observes the thread as finished — so there is a real window in which the session is already
    /// detached, the engine is gone, and the slot is already free. `detached` is the session value
    /// the caller holds, exactly as `tear_down_live_review` holds the one from `take_session`.
    struct OpenTeardownWindow {
        detached: LiveReviewSession,
        receiver: std::sync::mpsc::Receiver<LiveReviewCommand>,
    }

    impl OpenTeardownWindow {
        /// The dispatch's remaining legs and its batch claim, exactly as `start_batch_analysis`
        /// takes them: signal the detached owner, join it (`finish_live_review_teardown` does
        /// both), release its registry entry, then claim.
        fn claim_batch(
            &mut self,
            registry: &AnalysisJobRegistry,
            job_id: &str,
            ...
            finish_live_review_teardown(
                self.detached.commands.take(),
                self.detached.forwarder.take(),
                self.detached.job_id.take(),
                registry,
            );
            registry.try_insert_exclusive(AnalysisJobKind::Batch, job_id.to_string())
        }
    }

    /// Register `job_id`'s live session and leave it in the window a start's teardown passes through:
    /// detached from the state, with its registry entry already released.
    ///
    /// This is the state the P1 report describes — the slot is observably empty while the teardown
    /// that owns it is still mid-flight.
    fn open_live_teardown_window(
        ...
        let (commands, receiver) = std::sync::mpsc::channel();
        {
            ...
            live.forwarder = Some(std::thread::spawn(|| {}));
            live.job_id = Some(job_id.to_string());
        }
        registry
            .try_insert_exclusive(AnalysisJobKind::Live, job_id.to_string())
        ...
        let detached = state
            ...
            .unwrap()
            .take_session()
            .expect("the session was just registered");
        // The window: the session is detached and the owner's slot is released, while the caller has
        // not yet signalled or joined the owner, nor released anything itself.
        registry.remove(job_id);
        OpenTeardownWindow { detached, receiver }
    }

    /// The P1 regression: a single-shot claim must not take the batch slot inside a teardown window.
    ///
    /// The interleaving is forced, not left to the scheduler. `start_batch_analysis` runs under
    /// `lifecycle`, and the window it passes through — the displaced live owner has released its
    /// entry, the dispatch has not yet joined it — is genuinely empty, so a check-then-claim
    /// single-shot run would take the `Batch` slot there. On the old code that claim succeeded and
    /// the batch start then found the slot taken by *its own* kind: its `debug_assert_eq!` fired and
    /// panicked while the `lifecycle` guard was held, poisoning the lock for every later start.
    ///
    /// Because the claim now takes `lifecycle` for the claim itself, it cannot reach the slot while
    /// the dispatch holds that lock: it blocks on the mutex, and once it is released it sees the
    /// batch entry the dispatch installed and is refused cleanly. Both the refusal and the
    /// unpoisoned lock are asserted.
    #[test]
    fn single_shot_claim_cannot_enter_the_batch_teardown_window() {
        let registry = std::sync::Arc::new(AnalysisJobRegistry::default());
        let state = Mutex::new(LiveReviewSession {
            commands: None,
            forwarder: None,
            job_id: Some("live-1".to_string()),
        ...
        // The dispatch holds `lifecycle` for its whole sequence, and opens the window inside it.
        let _lifecycle = registry.lifecycle.lock().unwrap();
        let mut window = open_live_teardown_window(&state, &registry, "live-1");
        assert_eq!(
            registry.kind_of_first(),
            None,
            "the window really is the empty slot a check-then-claim run would observe"
        );

        // The concurrent single-shot request. It must not reach the slot this start reserved.
        let claim = {
            let registry = std::sync::Arc::clone(&registry);
            std::thread::spawn(move || claim_single_shot_slot(&registry, AnalysisJobKind::Batch))
        };
        // Every chance to reach the slot: only `lifecycle` can keep it out, and it must.
        std::thread::sleep(std::time::Duration::from_millis(100));
        assert_eq!(
            registry.kind_of_first(),
            None,
            "a single-shot claim must not take the slot while the start holds the lifecycle lock"
        );

        // The dispatch finishes its sequence: claim, then release the lock.
        let batch_slot = window
            .claim_batch(&registry, "batch-1")
            .expect("the dispatch claims the slot it reserved");
        drop(_lifecycle);

        let error = claim
            .join()
            .expect("the claim thread must not panic")
            .expect_err("the claim must be refused once the batch start owns the slot");
        assert!(error.contains("batch analysis"), "{error}");

        // Nothing panicked while the lock was held, so the lock is still usable — the poisoning the
        // old code caused is gone.
        assert!(
            registry.lifecycle.lock().is_ok(),
            "the lifecycle lock must not be poisoned"
        );
        assert!(matches!(
            window.receiver.try_recv(),
            Ok(LiveReviewCommand::Shutdown)
        ));
        assert!(registry.remove_if_owner("batch-1", &batch_slot));
        assert_eq!(registry.kind_of_first(), None);
    }

    /// The other order: the single-shot claim wins `lifecycle` and is still refused, not admitted.
    ///
    /// A single-shot run never displaces a live session, so when it gets the lock while the live
    /// session is still registered it must be refused with the live owner named — and it must leave
    /// the live slot alone for the batch start that is about to displace it.
    #[test]
    fn single_shot_claim_is_refused_by_a_live_owner_before_the_teardown() {
        let registry = AnalysisJobRegistry::default();
        let _live_slot = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-1".to_string())
            .unwrap();
    ...
    /// A live session is displaced by a batch start: `start_batch_analysis` must produce `Live -> Batch`.
    ///
    /// The old code claimed the batch slot *first* and only then tore the live session down, so with
    /// the live session holding the slot the claim always failed and Live->Batch was impossible.
    #[test]
    fn start_batch_analysis_displaces_a_live_session() {
        let registry = AnalysisJobRegistry::default();
        let (commands, receiver) = std::sync::mpsc::channel();
        ...
        let batch_slot = start_batch_analysis(&state, &registry, "batch-1".to_string())
            .expect("a live session must not block a batch start, it must be displaced");

        // The live session is gone: signalled, joined, and its own slot released.
        assert!(matches!(receiver.try_recv(), Ok(LiveReviewCommand::Shutdown)));
        assert!(!registry.contains("live-1"));
        // And the batch job now owns the exclusive slot.
        assert_eq!(registry.kind_of_first(), Some(AnalysisJobKind::Batch));
        assert!(registry.contains("batch-1"));
        // The state no longer reports a running session.
        assert!(!state.lock().unwrap().has_session());
        // The displaced slot is a different entry from the batch one.
        assert!(registry.remove_if_owner("batch-1", &batch_slot));
    }

    /// Restarting a live session must produce `Live -> Live`, not a refusal.
    ///
    /// `start_live_review_session` cannot be driven here (it needs a real `GtpSession`), so this
    /// mirrors its sequence — refuse a batch owner, displace a live owner, then claim — which is the
    /// part that was broken. The engine-spawn leg is covered separately by
    /// [`start_live_review_start_failure_releases_its_slot`].
    #[test]
    fn live_session_is_replaced_by_a_new_live_session() {
        let registry = AnalysisJobRegistry::default();
        let (commands, receiver) = std::sync::mpsc::channel();
        let owner = std::thread::spawn(|| {});
        let state = Mutex::new(LiveReviewSession {
        ...
        // Same sequence the live start performs: the existing kind must be Live (not Batch), the old
        // session is torn down and joined, then the new Live slot is claimed.
        assert_eq!(registry.kind_of_first(), Some(AnalysisJobKind::Live));
        tear_down_live_review(&state, &registry);
        let new_slot = registry
            .try_insert_exclusive(AnalysisJobKind::Live, "live-2".to_string())
            .unwrap();

        assert!(matches!(receiver.try_recv(), Ok(LiveReviewCommand::Shutdown)));
        assert!(!registry.contains("live-1"));
        assert_eq!(registry.kind_of_first(), Some(AnalysisJobKind::Live));
        assert!(registry.contains("live-2"));
        assert!(registry.remove_if_owner("live-2", &new_slot));
    }

    /// A batch job must refuse a live start: the live start path rejects a non-live owner.
    #[test]
    fn live_start_is_refused_while_a_batch_job_owns_the_slot() {
        let registry = AnalysisJobRegistry::default();
        let _batch = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "batch-1".to_string())
            .unwrap();

        // This is the guard `start_live_review_session` runs before it touches the live state.
        let error = match registry.kind_of_first() {
            None | Some(AnalysisJobKind::Live) => {
                panic!("a batch job must not present as absent or live")
            }
            Some(owner) => format!(
                "cannot start {} while a {} job is running",
        ...
        assert!(error.contains("live review"), "{error}");
        assert!(error.contains("batch analysis"), "{error}");
        // Nothing was displaced, and the batch slot is intact.
        assert!(registry.contains("batch-1"));
    }

    /// A live start whose engine never comes up must release the slot it claimed.
    ///
    /// The regression: `GtpSession::start` (or the `boardsize` handshake) failed *after* the slot was
    /// taken, and the early `?` returned without releasing it — so every later start was refused
    /// until the process exited. No real engine is needed: the release path is driven directly with
    /// the same slot the start would have claimed.
    #[test]
    fn start_live_review_start_failure_releases_its_slot() {
        let registry = AnalysisJobRegistry::default();
        let job_id = "live-fail".to_string();
        let slot = registry
            .try_insert_exclusive(AnalysisJobKind::Live, job_id.clone())
            .unwrap();

        // The failing start's cleanup, as `start_live_review_session` now performs it on both the
        // `GtpSession::start` and the `boardsize` error paths.
        let start_result: Result<(), String> = Err("failed to spawn katago".to_string());
        match start_result {
            Ok(()) => panic!("the fake engine was supposed to fail"),
            Err(_) => {
                assert!(registry.remove_if_owner(&job_id, &slot));
            }
        }

        // The slot is free again: a retry (or a batch run) must be able to start.
        assert!(!registry.contains(&job_id));
        assert_eq!(registry.kind_of_first(), None);
        assert!(registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "batch-after-failure".to_string())
            .is_ok());
    }

    /// The batch worker must release its slot *before* it emits its terminal event.
    ///
    /// Otherwise a frontend that restarts on `analysis-complete` / `analysis-error` is refused by a
    /// slot still held by a job whose engine has already exited. This asserts the ordering directly
    /// with a fake worker: the slot is dropped, then the terminal event is emitted, and a start from
    /// the event handler succeeds.
    #[test]
    fn batch_worker_releases_the_slot_before_emitting_its_terminal_event() {
        let registry = std::sync::Arc::new(AnalysisJobRegistry::default());
        let job_id = "batch-terminal".to_string();
        let slot = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, job_id.clone())
            .unwrap();

        let observed = std::sync::Arc::new(std::sync::Mutex::new(None));
        let registry_in_worker = std::sync::Arc::clone(&registry);
        let observed_in_worker = std::sync::Arc::clone(&observed);
        let worker_job_id = job_id.clone();

        // Stands in for `run_katago_analysis_job`: release, then emit.
        std::thread::spawn(move || {
            let released = registry_in_worker.remove_if_owner(&worker_job_id, &slot);
            assert!(released, "the worker must own the slot it releases");
            // The "emit": a frontend that starts the next job immediately.
            let start_outcome = registry_in_worker
                .try_insert_exclusive(AnalysisJobKind::Live, "live-after-complete".to_string());
            *observed_in_worker.lock().unwrap() = Some(start_outcome.is_ok());
        })
        .join()
        .unwrap();
        ...
        assert_eq!(
            *observed.lock().unwrap(),
            Some(true),
            "the slot must be free by the time the terminal event is emitted"
        );
    }

    /// Cancelling keeps the slot until the worker returns, then the slot is immediately reusable.
    #[test]
    fn cancelled_batch_job_holds_the_slot_until_it_returns() {
        let registry = AnalysisJobRegistry::default();
        let slot = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-1".to_string())
            .unwrap();

        assert!(cancel_analysis_job_in_registry(&registry, "job-1").unwrap());
        assert!(slot.cancel_token.is_cancelled());
        // Still held: a new job must not start on top of a cancelling engine.
        assert!(registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-2".to_string())
            .is_err());

        // The worker's return is what frees it.
        assert!(registry.remove_if_owner("job-1", &slot));
        assert!(registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "job-2".to_string())
            .is_ok());
    ...
    /// Two concurrent batch starts cannot both win the slot.
    ///
    /// The single-slot rule under contention, with real threads: exactly one claim succeeds.
    #[test]
    fn concurrent_batch_starts_elect_exactly_one_winner() {
        let registry = std::sync::Arc::new(AnalysisJobRegistry::default());
        ...
        let handles: Vec<_> = (0..8)
            .map(|i| {
                ...
                std::thread::spawn(move || {
                    if registry
                        .try_insert_exclusive(AnalysisJobKind::Batch, format!("job-{i}"))
                        .is_ok()
                    {
        ...
        assert_eq!(
            ...
            1,
            "the exclusive slot must admit exactly one batch job"
        );
    }

    /// Every registered job is cancelled on exit, and no slot is dropped behind the workers' backs.
    #[test]
    fn registry_cancel_all_signals_every_job_and_keeps_their_slots() {
        let registry = AnalysisJobRegistry::default();
        let batch = registry
            .try_insert_exclusive(AnalysisJobKind::Batch, "batch-1".to_string())
            .unwrap();
        ...
        assert_eq!(cancelled, vec!["batch-1".to_string()]);
        assert!(batch.cancel_token.is_cancelled());
        assert!(registry.contains("batch-1"), "the worker still owns the slot");
    }
    ...
    fn analysis_cache_saves_and_restores_payload_from_sqlite() {
        ...
        let problems = classify_problems(frames.clone(), Some(sgf_text.to_string())).unwrap();
        let payload = serde_json::json!({
            "frames": frames,
            "problems": problems,
        });
