import type { LiveSearchOptions } from "./liveControls";
...
export async function analyzeKataGoOnce(profile: EngineProfileDto, sgfText: string, turn: number, maxVisits: number): Promise<AnalysisFrameDto> {
  if (!isTauriRuntime()) {
    throw new Error("真实分析需要 Tauri 桌面应用和 KataGo，浏览器仅供预览。");
  }
...
export async function analyzeKataGoGame(profile: EngineProfileDto, sgfText: string, maxVisits: number): Promise<AnalysisFrameDto[]> {
  if (!isTauriRuntime()) {
    throw new Error("全盘分析需要 Tauri 桌面应用和 KataGo，浏览器仅供预览。");
  }
...
export type LiveReviewFramePayload = {
  ...
  turn: number;
  /**
   * Side to move in this frame's position. **Metadata only**: the values inside `frame` are
   * already black-perspective, so do NOT convert again from this field.
   */
  to_play: PlayerColor;
  /**
   * **Already converted to black perspective** (plan 005 Step 3b), matching the batch path.
   * Render `winrate_black`/`score_mean_black` directly; a second flip would invert every
   * white-to-play frame.
   */
...
export async function startLiveReview(
  ...
  boardSize: number,
  turn: number,
  intervalCentisec: number
): Promise<string> {
  if (!isTauriRuntime()) return "";
  return await invoke<string>("katago_start_live_review", { profile, boardSize, turn, intervalCentisec });
}

export async function setLiveReviewPosition(
  moves: string[],
  turn: number,
  intervalCentisec: number,
  player?: PlayerColor,
  ...
  return await invoke<number>("katago_live_review_set_position", {
    moves,
    turn,
    intervalCentisec,
    player,
    komi, boardSize, setup, rules, handicap, options
  });
}

export async function stopLiveReview(): Promise<void> {
  if (!isTauriRuntime()) return;
  await invoke<void>("katago_live_review_stop");
}

export async function listenToLiveReviewEvents(handlers: LiveReviewEventHandlers): Promise<() => void> {
  if (!isTauriRuntime()) return () => undefined;
  const unlisteners = await Promise.all([
    listen<LiveReviewFramePayload>("katago://live-review-frame", (event) => handlers.onFrame?.(event.payload)),
    listen<LiveReviewEndedPayload>("katago://live-review-ended", (event) => handlers.onEnded?.(event.payload))
  ]);
  return () => {
    for (const unlisten of unlisteners) unlisten();
  };
...
export async function saveEngineProfilesSettings(settings: EngineProfilesSettingsDto): Promise<EngineProfilesSettingsDto> {
  ...
  const saved = await invoke<EngineProfilesSettingsDto>("save_engine_profiles_settings", { settings });
  window.dispatchEvent(new Event("engine-profiles-changed"));
  return saved;
}
...
export async function classifyProblems(frames: AnalysisFrameDto[], sgfText?: string): Promise<ProblemMarkerDto[]> {
  // Browser previews do not infer problem moves from incomplete or recorded data.
  if (!isTauriRuntime()) return [];
  return await invoke<ProblemMarkerDto[]>("classify_problems", { frames, sgfText: sgfText ?? null });
}
