...
function cacheRecordMetadata(record: AnalysisCacheRecord): string {
  ...
  const frameDetail = record.analyzedMoveCount > record.moveCount ? ` (${record.analyzedMoveCount} 帧)` : "";
  const engineText = record.engineKind === "katago" ? "KataGo" : "模拟引擎";
  const parts = [
    engineText,
    `${analyzedMoves}/${record.moveCount} 手${frameDetail}`,
