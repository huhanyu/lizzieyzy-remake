...
// Temporary visualization: copy the board without changing SGF or live analysis.
export function previewVariation(position: PositionDto, pv: MoveVertex[]): Stone[] {
  const stones = new Map<string, StoneDto>(position.stones.map(stone => [pointKey(stone.x, stone.y), { ...stone }]));
  const numbers = new Map<string, number>();
  let color = position.to_play;
  for (const [index, vertex] of pv.entries()) {
    if (isPoint(vertex)) {
      const { x, y } = vertex.point;
      if (!isOnBoard(x, y, position.board_size) || stones.has(pointKey(x, y))) break;
      const before = new Map(stones);
      stones.set(pointKey(x, y), { x, y, color });
      const captured = captureAdjacentOpponentGroups(stones, x, y, color, position.board_size);
      const group = collectGroup(stones, x, y, position.board_size)!;
      if (countLiberties(stones, group, position.board_size) === 0) {
        stones.clear(); before.forEach((stone, key) => stones.set(key, stone)); break;
      }
      captured.forEach(stone => numbers.delete(pointKey(stone.x, stone.y)));
      numbers.set(pointKey(x, y), index + 1);
    }
    color = color === "black" ? "white" : "black";
  }
  return [...stones.values()].map(stone => ({ ...stone, moveNumber: numbers.get(pointKey(stone.x, stone.y)) ?? 0 }));
}
