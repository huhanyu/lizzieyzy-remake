...
pub struct GameSummaryDto {
    ...
    /// Initial setup stones (SGF `AB`/`AW`/`AE`), coordinates are 0-based. Empty means an empty board.
    #[serde(default)]
    pub initial_stones: Vec<StoneDto>,
    /// Initial side to play (SGF `PL`). Defaults to black.
    #[serde(default)]
    pub to_play: PlayerColor,
    /// Raw rules text from the SGF `RU` property (free text, may be localized).
    /// Normalize with the rules mapping before handing it to an engine.
    #[serde(default)]
