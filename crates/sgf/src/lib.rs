...
/// Outcome of committing a [`TryPlaySession`] back into a document.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SaveTryPlayResult {
    /// The source document with the try-play line appended as an extra branch.
    pub sgf_text: String,
    /// The last try-play node, or the anchor node when no move was played.
    pub node_id: NodeId,
}
...
pub enum SgfError {
    ...
    #[error("try-play session is stale: the source SGF changed since the session started")]
    StaleTryPlaySession,
}
...
pub fn parse_sgf(input: &str) -> Result<SgfDocument, SgfError> {
    ...
    let komi = property_values(&root, "KM")
        .and_then(|v| v.first())
        .and_then(|v| komi::imported_komi(v, property_values(&root, "AP").and_then(|values| values.first()).map(String::as_str)))
        .unwrap_or(7.5);
...
/// A temporary "try play" line rooted at an anchor node of a source document.
///
/// The session owns a private SGF copy of the source; nothing is written back to
/// the caller's document until [`TryPlaySession::save`] is called explicitly.
/// Entering and exiting therefore cannot mutate the original record.
///
/// The side to play is always taken from the replayed position (`replay.to_play`),
/// never supplied by the caller. Node ids are document-local, so a session is
/// additionally bound to the exact source text it was opened from; saving against
/// a source that changed in the meantime fails with [`SgfError::StaleTryPlaySession`].
#[derive(Debug, Clone)]
pub struct TryPlaySession {
    source_sgf: String,
...
impl TryPlaySession {
    /// Enter a try-play line at `anchor_node_id` of `source_sgf`.
    ///
    /// Fails if the anchor does not exist, or if the anchor's position cannot be
    /// replayed (the source itself contains an illegal line).
    pub fn begin(source_sgf: &str, anchor_node_id: NodeId) -> Result<Self, SgfError> {
        let anchor_position = replay_sgf_position_at_node(source_sgf, anchor_node_id)?;
        if !anchor_position.errors.is_empty() {
            return Err(SgfError::IllegalMove(anchor_position.errors.join("; ")));
        }
        Ok(Self {
            source_sgf: source_sgf.to_string(),
            anchor_node_id,
            working_sgf: source_sgf.to_string(),
            current_node_id: anchor_node_id,
            moved: false,
        })
    }

    /// The node the session was entered at.
    pub fn anchor_node_id(&self) -> NodeId {
        self.anchor_node_id
    }

    /// The node the current try-play line ends at.
    pub fn current_node_id(&self) -> NodeId {
        self.current_node_id
    }

    /// Whether at least one try-play move has been appended.
    pub fn has_moves(&self) -> bool {
        self.moved
    }

    /// The private working copy of the source document, including try-play moves.
    pub fn working_sgf(&self) -> &str {
        &self.working_sgf
    }

    /// The side to play in the current try-play position.
    pub fn to_play(&self) -> Result<PlayerColor, SgfError> {
        Ok(replay_sgf_position_at_node(&self.working_sgf, self.current_node_id)?.to_play)
    }

    /// The current try-play position, including captures and any rule errors.
    pub fn position(&self) -> Result<PositionDto, SgfError> {
        replay_sgf_position_at_node(&self.working_sgf, self.current_node_id)
    }

    /// Play a try-play move with the side the replay says is to move.
    ///
    /// Illegal moves are rejected and leave the session unchanged. The color is
    /// never taken from the caller.
    ///
    /// The append is replayed against the new node before the session state is
    /// touched, so a move that serializes but does not replay cleanly fails without
    /// advancing the line.
    pub fn play(&mut self, vertex: MoveVertex) -> Result<PositionDto, SgfError> {
        let to_play = self.to_play()?;
        let result = append_sgf_move(&self.working_sgf, self.current_node_id, to_play, vertex)?;
        let position = replay_sgf_position_at_node(&result.sgf_text, result.new_node_id)?;
        if !position.errors.is_empty() {
            return Err(SgfError::IllegalMove(position.errors.join("; ")));
        }

        self.working_sgf = result.sgf_text;
        self.current_node_id = result.new_node_id;
        self.moved = true;
        Ok(position)
    }

    /// Play a try-play pass with the side the replay says is to move.
    pub fn play_pass(&mut self) -> Result<PositionDto, SgfError> {
        self.play(MoveVertex::Pass)
    }

    /// Drop the whole try-play line and return to the anchor node.
    ///
    /// The session is reusable afterwards; the original document was never touched.
    pub fn exit(&mut self) {
        self.working_sgf = self.source_sgf.clone();
        self.current_node_id = self.anchor_node_id;
        self.moved = false;
    }

    /// Commit the try-play line as an extra branch of `source_sgf`.
    ///
    /// The working copy was built from `source_sgf` by validated appends, so once the
    /// caller's text is confirmed byte-identical to the text the session was opened
    /// from, the working copy is the committed document and its current node is the
    /// end of the try-play line. Bytes equal means the only difference between the
    /// two is the appended branch, so no re-application is needed.
    ///
    /// When the text differs the session's node ids can no longer be trusted and this
    /// fails with [`SgfError::StaleTryPlaySession`]. When no move was played the
    /// working copy still equals the source, so the source is returned unchanged.
    pub fn save(&self, source_sgf: &str) -> Result<SaveTryPlayResult, SgfError> {
        if source_sgf != self.source_sgf {
            return Err(SgfError::StaleTryPlaySession);
        }
        Ok(SaveTryPlayResult {
            sgf_text: self.working_sgf.clone(),
...
pub fn to_game_dto(doc: SgfDocument) -> GameDto {
    let (initial_stones, to_play) = setup_stones_and_player_to_play(&doc);
    // Raw SGF `RU` text. `katago_rules_for_sgf_ru` normalizes it at the query-construction boundary.
    let rules = doc
        .root
        .as_ref()
...
/// Collect the setup stones (`AB`/`AW`/`AE`) and the player to play (`PL`) declared by the leading
/// non-move nodes of the mainline, reusing the same node-by-node walk as [`replay_sgf_positions`].
///
/// Unlike the replay path this degrades to an empty board instead of failing: `to_game_dto` returns
/// no `Result`, and the SGF has already been validated by `parse_sgf`, so a malformed setup property
/// here is an exceptional input where an empty board beats failing the whole command.
fn setup_stones_and_player_to_play(document: &SgfDocument) -> (Vec<StoneDto>, PlayerColor) {
    let Some(root) = document.root.as_ref() else {
        return (Vec::new(), PlayerColor::Black);
    };
    let Ok(mut board) = Board::new(document.board_size) else {
        return (Vec::new(), PlayerColor::Black);
    };
    ...
    for node in mainline_nodes(root) {
        ...
        if apply_setup_properties(&mut board, node, document.board_size).is_err() {
            return (Vec::new(), PlayerColor::Black);
        }
        if let Ok(Some(color)) = player_to_play(node) {
            to_play = color;
        }
    }

    (stones_from_board(&board), to_play)
}

/// Map the free-text SGF `RU` property onto a rule name KataGo's analysis engine accepts.
///
/// `RU` is free text (`Chinese`, `Japanese rules`, `中国规则`, ...), while KataGo expects a lowercase
/// identifier. Unknown or missing values fall back to `chinese`, which is the historical default of
/// the analysis query path.
pub fn katago_rules_for_sgf_ru(raw: Option<&str>) -> &'static str {
    let Some(raw) = raw else {
        return "chinese";
    };
    let lowered = raw.trim().to_ascii_lowercase();
    let normalized = lowered.trim_end_matches(" rules").trim();

    match normalized {
        "chinese" | "中国" | "中国规则" | "cn" | "zh" => "chinese",
        "japanese" | "日本" | "日本规则" | "jp" | "ja" => "japanese",
        "korean" | "韩国" | "韩国规则" | "kr" | "ko" => "korean",
        "tromp-taylor" | "tromp taylor" | "tt" => "tromp-taylor",
        _ => "chinese",
    }
}
...
mod tests {
    ...
    #[test]
    fn to_game_dto_carries_setup_stones_and_player_to_play() {
        let input = include_str!("../../../tests/golden/handicap_9x9.sgf");
        let game = to_game_dto(parse_sgf(input).unwrap());

        assert_eq!(game.summary.board_size, 9);
        assert_eq!(game.summary.initial_stones.len(), 3);
        assert!(has_dto_stone(
            &game.summary.initial_stones,
            2,
            2,
            PlayerColor::Black
        ));
        ...
        assert_eq!(game.summary.to_play, PlayerColor::White);
        // `rules` carries the raw SGF RU text; normalization happens at the query boundary.
        assert_eq!(game.summary.rules.as_deref(), Some("Japanese"));
        assert_eq!(katago_rules_for_sgf_ru(game.summary.rules.as_deref()), "japanese");
        assert_eq!(game.moves.len(), 3);
    }

    #[test]
    fn to_game_dto_defaults_to_empty_board_for_normal_game() {
        let input = include_str!("../../../tests/golden/basic_19x19.sgf");
        let game = to_game_dto(parse_sgf(input).unwrap());

        assert!(game.summary.initial_stones.is_empty());
        assert_eq!(game.summary.to_play, PlayerColor::Black);
        // basic_19x19.sgf carries no RU property, so no rules override is produced.
        assert_eq!(game.summary.rules, None);
    }

    #[test]
    fn katago_rules_for_sgf_ru_maps_known_aliases() {
        assert_eq!(katago_rules_for_sgf_ru(Some("Chinese")), "chinese");
        assert_eq!(katago_rules_for_sgf_ru(Some("中国")), "chinese");
        assert_eq!(katago_rules_for_sgf_ru(Some("Japanese")), "japanese");
        assert_eq!(katago_rules_for_sgf_ru(Some("Japanese rules")), "japanese");
        assert_eq!(katago_rules_for_sgf_ru(Some("日本")), "japanese");
        assert_eq!(katago_rules_for_sgf_ru(Some("Tromp-Taylor")), "tromp-taylor");
        assert_eq!(katago_rules_for_sgf_ru(Some("tt")), "tromp-taylor");
        assert_eq!(katago_rules_for_sgf_ru(Some("korean")), "korean");
        // Unknown values fall back to the historical default.
        assert_eq!(katago_rules_for_sgf_ru(Some("new-zealand")), "chinese");
    }

    #[test]
    fn katago_rules_for_sgf_ru_falls_back_to_chinese_for_missing_or_blank_text() {
        assert_eq!(katago_rules_for_sgf_ru(None), "chinese");
        assert_eq!(katago_rules_for_sgf_ru(Some("")), "chinese");
        assert_eq!(katago_rules_for_sgf_ru(Some("   ")), "chinese");
    }

    fn try_play_anchor(sgf: &str, move_number: u32) -> NodeId {
        let tree = to_sgf_tree_dto(&parse_sgf(sgf).unwrap()).unwrap().unwrap();
        tree.nodes
            .iter()
            .find(|node| node.move_number == Some(move_number))
            .unwrap()
            .id
    }

    #[test]
    fn try_play_does_not_touch_the_source_until_saved() {
        let source = "(;SZ[5];B[aa]C[first])";
        let anchor = try_play_anchor(source, 1);

        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        assert_eq!(session.anchor_node_id(), anchor);
        assert!(!session.has_moves());
        assert_eq!(session.to_play().unwrap(), PlayerColor::White);

        let position = session.play(MoveVertex::Point(PointDto { x: 1, y: 1 })).unwrap();
        assert_eq!(position.move_number, 2);
        assert!(position.errors.is_empty());
        assert!(has_stone(&position, 1, 1, PlayerColor::White));

        // The caller's document is untouched while the session is open.
        assert_eq!(source, "(;SZ[5];B[aa]C[first])");
        assert_eq!(parse_sgf(source).unwrap().moves.len(), 1);

        // Exiting restores the anchor position and drops the try-play line.
        session.exit();
        assert!(!session.has_moves());
        assert_eq!(session.current_node_id(), anchor);
        assert_eq!(session.working_sgf(), source);
        let restored = session.position().unwrap();
        assert_eq!(restored.move_number, 1);
        assert_eq!(restored.to_play, PlayerColor::White);
        assert!(!has_stone(&restored, 1, 1, PlayerColor::White));
    }

    #[test]
    fn try_play_uses_replay_to_play_and_ignores_any_caller_color() {
        // White is to play at the anchor; the session must play White.
        let source = "(;SZ[5]PL[W];B[aa])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();

        let position = session.play(MoveVertex::Point(PointDto { x: 2, y: 2 })).unwrap();

        assert!(has_stone(&position, 2, 2, PlayerColor::White));
        assert!(session.working_sgf().contains(";W[cc]"));
    }

    #[test]
    fn try_play_captures_and_reports_captured_stones() {
        // White at (1,1) has three black neighbours and one liberty at (1,2).
        let source = "(;SZ[5]AB[ab][ba][cb]AW[bb];B[dd]PL[B])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        assert_eq!(session.to_play().unwrap(), PlayerColor::Black);

        let position = session.play(MoveVertex::Point(PointDto { x: 1, y: 2 })).unwrap();

        assert!(position.errors.is_empty());
        assert_eq!(position.captures_black, 1);
        assert!(!has_stone(&position, 1, 1, PlayerColor::White));
        assert_eq!(position.to_play, PlayerColor::White);
    }

    #[test]
    fn try_play_rejects_illegal_moves_and_keeps_the_session_usable() {
        let source = "(;SZ[5];B[aa])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        ...
        let off_board = session
            .play(MoveVertex::Point(PointDto { x: 9, y: 9 }))
            .unwrap_err();
        assert!(matches!(off_board, SgfError::IllegalMove(_)));

        // Still at the anchor, still able to play a legal move.
        assert!(!session.has_moves());
        assert_eq!(session.current_node_id(), anchor);
        let position = session.play(MoveVertex::Point(PointDto { x: 1, y: 1 })).unwrap();
        assert!(has_stone(&position, 1, 1, PlayerColor::White));
    ...
    #[test]
    fn try_play_rejects_immediate_recapture_under_simple_ko() {
        // Ko shape on a 5x5, stated as a standalone fixture so the position is readable
        // without replaying a move list (SGF letters are x=file, y=rank):
        //   . . . . .      AB[ab][ba][cb] AW[bb][ac][cc][bd] PL[B] — Black to play.
        //   B W B . .      White bb has its single liberty at bc; Black bc captures it.
        //   W . W . .      The capturing stone then has a single liberty at bb, so
        //   . W . . .      White's immediate recapture at bb is the simple-ko move.
        //   . . . . .
        let source = "(;SZ[5]AB[ab][ba][cb]AW[bb][ac][cc][bd]PL[B])";
        let anchor = to_sgf_tree_dto(&parse_sgf(source).unwrap())
            .unwrap()
            .unwrap()
            .root_id;

        let anchor_position = replay_sgf_position_at_node(source, anchor).unwrap();
        assert!(anchor_position.errors.is_empty());
        assert_eq!(anchor_position.to_play, PlayerColor::Black);
        assert!(has_stone(&anchor_position, 1, 1, PlayerColor::White));
        assert!(has_stone(&anchor_position, 2, 1, PlayerColor::Black));

        let mut session = TryPlaySession::begin(source, anchor).unwrap();

        // Black bc captures the single white stone, after which White is to play.
        let capture = session.play(MoveVertex::Point(PointDto { x: 1, y: 2 })).unwrap();
        assert!(capture.errors.is_empty());
        assert_eq!(capture.captures_black, 1);
        assert_eq!(capture.to_play, PlayerColor::White);
        assert!(!has_stone(&capture, 1, 1, PlayerColor::White));
        assert!(has_stone(&capture, 1, 2, PlayerColor::Black));
        let capture_node = session.current_node_id();

        // Prove the recapture is a legal White move on the *capture* position apart from
        // the ko rule (same stones, no ko point): otherwise the assertion below would
        // only show that some illegal move was rejected, not that a ko was rejected.
        let mut recapture_board = Board::new(5).unwrap();
        for (x, y, color) in [
            (0u8, 1u8, Color::Black),
            (1, 0, Color::Black),
            (2, 1, Color::Black),
            ...
            recapture_board.set_stone(Point { x, y }, Some(color)).unwrap();
        }
        assert!(
            recapture_board
                .play(Color::White, Vertex::Point(Point { x: 1, y: 1 }))
                .is_ok(),
            "the fixture's recapture must be legal apart from the ko rule"
        );

        let ko = session
            .play(MoveVertex::Point(PointDto { x: 1, y: 1 }))
            .unwrap_err();
        assert!(matches!(ko, SgfError::IllegalMove(message) if message.contains("ko")));
        // The rejected move did not advance the line.
        assert_eq!(session.current_node_id(), capture_node);
        assert_eq!(session.position().unwrap().captures_black, 1);
    }

    #[test]
    fn try_play_supports_pass_and_alternates_side_to_play() {
        let source = "(;SZ[5];B[aa])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();

        let after_pass = session.play_pass().unwrap();
        assert!(matches!(
            after_pass.last_move.as_ref().unwrap().vertex,
            MoveVertex::Pass
        ));
        assert_eq!(after_pass.to_play, PlayerColor::Black);
        assert!(session.working_sgf().ends_with(";W[])"));

        let after_black = session.play(MoveVertex::Point(PointDto { x: 1, y: 1 })).unwrap();
        assert!(has_stone(&after_black, 1, 1, PlayerColor::Black));
        assert_eq!(after_black.to_play, PlayerColor::White);
    }

    #[test]
    fn try_play_nested_variation_saves_as_a_new_branch_without_overwriting_siblings() {
        let source = "(;SZ[5];B[aa](;W[bb]C[main])(;W[cc]C[branch]))";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 3, y: 3 })).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 4, y: 4 })).unwrap();

        let saved = session.save(source).unwrap();

        let tree = to_sgf_tree_dto(&parse_sgf(&saved.sgf_text).unwrap())
            .unwrap()
            .unwrap();
        let parent = tree.nodes.iter().find(|node| node.id == anchor).unwrap();
        assert_eq!(parent.child_ids.len(), 3);
        let saved_node = tree.nodes.iter().find(|node| node.id == saved.node_id).unwrap();
        assert_eq!(saved_node.move_number, Some(3));
        // White moves first from the anchor, so the second try-play move is Black.
        assert_eq!(saved_node.color, Some(PlayerColor::Black));
        assert_eq!(
            saved_node.vertex,
            Some(MoveVertex::Point(PointDto { x: 4, y: 4 }))
        );
        assert!(!saved_node.is_mainline);

        // Existing variations and comments survive the save.
        assert!(saved.sgf_text.contains("(;W[bb]C[main])"));
        assert!(saved.sgf_text.contains("(;W[cc]C[branch])"));

        // The saved document reopens with the branch readable at its node.
        let reopened = replay_sgf_position_at_node(&saved.sgf_text, saved.node_id).unwrap();
        assert_eq!(reopened.move_number, 3);
        assert!(has_stone(&reopened, 3, 3, PlayerColor::White));
        assert!(has_stone(&reopened, 4, 4, PlayerColor::Black));
        assert!(reopened.errors.is_empty());

        // The source text the caller still holds is unchanged.
        assert_eq!(source, "(;SZ[5];B[aa](;W[bb]C[main])(;W[cc]C[branch]))");
    }

    #[test]
    fn try_play_save_rejects_a_source_that_changed_since_begin() {
        let source = "(;SZ[5];B[aa])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 1, y: 1 })).unwrap();

        let changed = "(;SZ[5];B[aa]C[edited])";
        let error = session.save(changed).unwrap_err();

        assert!(matches!(error, SgfError::StaleTryPlaySession));
        // The stale document was not modified.
        assert_eq!(changed, "(;SZ[5];B[aa]C[edited])");
    }

    #[test]
    fn try_play_save_without_moves_returns_the_source_unchanged() {
        let source = "(;SZ[5];B[aa])";
        let anchor = try_play_anchor(source, 1);
        let session = TryPlaySession::begin(source, anchor).unwrap();

        let saved = session.save(source).unwrap();

        assert_eq!(saved.sgf_text, source);
        assert_eq!(saved.node_id, anchor);
    }

    #[test]
    fn try_play_save_commits_the_working_copy_without_replaying() {
        // The committed document must be the session's working copy exactly, at the
        // session's current node. Guards against save re-deriving the line from the
        // source, which would silently diverge from the text the caller previewed.
        let source = "(;SZ[5];B[aa]C[first])";
        let anchor = try_play_anchor(source, 1);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 1, y: 1 })).unwrap();
        session.play_pass().unwrap();

        let working = session.working_sgf().to_string();
        let current = session.current_node_id();
        let saved = session.save(source).unwrap();

        assert_eq!(saved.sgf_text, working);
        assert_eq!(saved.node_id, current);
        // Saving neither disturbs the session nor the caller's document.
        assert_eq!(session.working_sgf(), working);
        assert_eq!(source, "(;SZ[5];B[aa]C[first])");
    }

    #[test]
    fn try_play_begin_rejects_unknown_anchor() {
        let source = "(;SZ[5];B[aa])";
        let error = TryPlaySession::begin(source, Uuid::new_v4()).unwrap_err();
        assert!(matches!(error, SgfError::NodeNotFound));
    }

    #[test]
    fn try_play_exit_discards_moves_without_mutating_the_source() {
        let source = "(;SZ[5];B[aa]C[keep];W[bb])";
        let anchor = try_play_anchor(source, 2);
        let mut session = TryPlaySession::begin(source, anchor).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 3, y: 3 })).unwrap();
        session.play(MoveVertex::Point(PointDto { x: 4, y: 4 })).unwrap();
        ...
        assert_eq!(session.working_sgf(), source);
        assert_eq!(session.position().unwrap().move_number, 2);
        let saved = session.save(source).unwrap();
        assert_eq!(saved.sgf_text, source);
        assert_eq!(saved.node_id, anchor);
    }

    fn has_dto_stone(stones: &[StoneDto], x: u8, y: u8, color: PlayerColor) -> bool {
        stones
            .iter()
